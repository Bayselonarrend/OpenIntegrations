...
Процедура СоздатьНаборWorkflow(Знач Языки, Знач КаталогПараметров, Знач КаталогWorkflow) Экспорт
	...
	ПутьШаблонаOsRpm      = КаталогШаблонов + "os_test_rpm.txt";
	...
	ШаблонOsRpm = Новый ДвоичныеДанные(ПутьШаблонаOsRpm);
	ШаблонOsRpm = ПолучитьСтрокуИзДвоичныхДанных(ШаблонOsRpm);
...
Процедура СформироватьWorflow(Знач Параметры, Знач КаталогWorkflow, Знач Язык)
	...
	ТекстJFOSRPM = СтрШаблон(ШаблонOsRpm, Язык, ТекстВыполненияOS);
