...
Процедура РазобратьМодуль(Модуль)
    ...
    СпсиокЗависимостей = Новый Массив;

    Для Н = 1 По ДокументМодуля.КоличествоСтрок() Цикл

        ТекущаяСтрока = СокрЛП(ДокументМодуля.ПолучитьСтроку(Н));
        Если СтрНачинаетсяС(ТекущаяСтрока, "// Depends:") Тогда
            СтрокаЗависимостей = СтрЗаменить(ТекущаяСтрока, "// Depends:", "");
            СтрокаЗависимостей = СокрЛП(СтрокаЗависимостей);
            СпсиокЗависимостей = СтрРазделить(СтрокаЗависимостей, ",", Ложь);
            Прервать;
        КонецЕсли;
    ...
    Расположение = ?(Команда = "tools", "/oint-cli/tools/Modules/", "/oint/core/Modules/");
    ЗаписатьФункциюПолученияСтрокиПодключения(СпсиокЗависимостей, Команда, Модуль.ИмяБезРасширения, Расположение);
...
Процедура ЗаписатьФункциюПолученияСтрокиПодключения(СпсиокЗависимостей, Библиотека, ИмяМодуля, Расположение)
    ...
    ОсновнойТекстПодключения    = "%1 = ЗагрузитьСценарий(""""%%1" + Расположение + "%1.os"""");";
    РасширенныйТекстПодключения = "%1 = ЗагрузитьСценарий(""""%%1" + Расположение + "%1.os"""", Контекст);";
    РекурсивныйТекстПодключения = "%1 = ЗагрузитьСценарий(""""%%1" + Расположение + "%1.os"""", Новый Структура(""%1"", Неопределено));";
    ...
    Иначе
        ...
        Для Каждого Зависимость Из СпсиокЗависимостей Цикл
            ...
            НеобходимыйШаблон        = ?(ИмяМодуля = Зависимость, РекурсивныйТекстПодключения, ОсновнойТекстПодключения);
            ТекущаяСтрокаПодключения = СтрШаблон(НеобходимыйШаблон, Зависимость);

            ТекущийФайлСостава.ДобавитьСтроку(СтрШаблон("        | %1", ТекущаяСтрокаПодключения));
            ТекущийФайлСостава.ДобавитьСтроку(СтрШаблон("        | Контекст.Вставить(""""%1"""", %1);", Зависимость));
