...
Процедура РазобратьМодуль(Модуль)
    ...
    ОпределитьСписокЗависимостей(ТекстМодуля, Модуль.ИмяБезРасширения, СпсиокЗависимостей);

    ЗаписатьФункциюПолученияСтрокиПодключения(СпсиокЗависимостей, Команда);

КонецПроцедуры

Процедура ОпределитьСписокЗависимостей(ТекстМодуля, ИмяМодуля, СтруктураЗависимостей)

    Для Каждого Модуль Из ПолныйМассивМодулей Цикл

        ИскомаяЗависимость = Модуль.ИмяБезРасширения;

        Если СтрНайти(ТекстМодуля, ИскомаяЗависимость) > 0 Или ИскомаяЗависимость = ИмяМодуля Тогда
            ...
            СтруктураЗависимостей.Вставить(Модуль.ИмяБезРасширения, НовыйПуть);
...
Процедура ОпределитьСоответствиеМодулейКоманд(ТекущийЯзык)
    ...
    ПолныйМассивМодулей = Новый Массив;
    ...
    Для Каждого Модуль Из ФайлыМодулей Цикл
        ...
        Если СтрНачинаетсяС(НазваниеМодуля, "OPI_") Или НазваниеМодуля = "Utils" Тогда
            ПолныйМассивМодулей.Добавить(Модуль);
        КонецЕсли;
