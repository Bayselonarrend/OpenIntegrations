...
Процедура РазобратьМодуль(Модуль)
    ...
    ЗаписатьФункциюПолученияСтрокиПодключения(СпсиокЗависимостей, Команда);
...
Процедура ЗаписатьФункциюПолученияСтрокиПодключения(СпсиокЗависимостей, Библиотека)
    ...
    ОсновнойТекстПодключения    = "%1 = ЗагрузитьСценарий(""""%%1/oint/core/Modules/%1.os"""")";
    РасширенныйТекстПодключения = "%1 = ЗагрузитьСценарий(""""%%1/oint/core/Modules/%1.os"""", Контекст)";

    Если Не ЗначениеЗаполнено(СпсиокЗависимостей) Тогда

        ТекстПодключения = СтрШаблон(ОсновнойТекстПодключения, Библиотека);
        ТекстФункции     = СтрШаблон("    Возврат ""%1""", ОсновнойТекстПодключения);
    ...
    Иначе
        ...
        Для Каждого Зависимость Из СпсиокЗависимостей Цикл
            ...
            ТекущаяСтрокаПодключения = СтрШаблон(ОсновнойТекстПодключения, Зависимость);
        ...
        ТекстПодключения = СтрШаблон(РасширенныйТекстПодключения, Библиотека);
        ТекущийФайлСостава.ДобавитьСтроку("        |  %1"";", ТекстПодключения);
