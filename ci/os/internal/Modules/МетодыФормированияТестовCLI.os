...
Процедура ПреобразоватьТестыВCLI(Знач Язык)
	...
	МассивЗамен = ПолучитьМассивЗаменЗаменСтрокОсновногоМодуля();
	...
	Для Каждого Путь Из ПутиСохраненияМодулейCLI Цикл
		ДокументТестов.Записать(Путь);
	КонецЦикла;

КонецПроцедуры

Функция ПолучитьМассивЗаменЗаменСтрокОсновногоМодуля()
	...
	Для Каждого Объявление Из СтруктураМодуля.Объявления Цикл
		...
		Если Объявление.Тип = "ОбъявлениеМетода" Тогда
			...
			Для Каждого Оператор Из Объявление.Операторы Цикл
				...
				ОтборСостава     = Новый Структура("Библиотека,МетодПоиска", Команда, вРег(Метод));
				...
				Если СписокПараметров.Количество() = 0 Тогда
					...
					МассивЗамен.Добавить(Новый Структура("Начало,Конец,ТекстЗамены", Объявление.Начало.НомерСтроки + 1, Объявление.Конец.НомерСтроки - 1, ""));
					Продолжить;
				КонецЕсли;
				...
				ТекстЗамены = ТекстЗамены + СтрШаблон("    %1 = OPI_ПолучениеДанныхТестов.ВыполнитьТестCLI(""%2"", ""%3"", Опции);", ИмяПеременной, Команда, Метод);
