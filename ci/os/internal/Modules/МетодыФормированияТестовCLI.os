...
Процедура ОбработатьОператор(Оператор, Язык, МассивЗамен, Отступ = "    ")
	...
	ИначеЕсли Оператор.Тип = "ОператорПрисваивания" Тогда
		...
		ТекстЗамены = Отступ + "Опции = Новый Структура;";
		...
		Для Н = 0 По Параметры.ВГраница() Цикл
			...
			ТекстЗамены = ТекстЗамены + Символы.ПС + СтрШаблон(Отступ + "Опции.Вставить(""%1"", %2);", ИмяПараметра, ЗначениеПараметра);

		КонецЦикла;

		ТекстЗамены = ТекстЗамены + Символы.ПС + Символы.ПС;
		ТекстЗамены = ТекстЗамены 
			+ СтрШаблон(Отступ + "%1 = OPI_ПолучениеДанныхТестов.ВыполнитьТестCLI(""%2"", ""%3"", Опции);", ИмяПеременной, Команда, Метод)
			+ Символы.ПС;
