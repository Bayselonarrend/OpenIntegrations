...
Процедура ЗаполнитьСекретныеДанные()
	...
	ПутьRO = СтрШаблон("file:%1?mode=ro", РасшифрованныйФайл.Путь);
	
	Таблица = "test_params";
	
    Результат = OPI_SQLite.ПолучитьЗаписи(Таблица, , , , , ПутьRO);
    
    Если Не Результат["result"] = Истина Тогда
    	ВызватьИсключение Результат["error"];
    КонецЕсли;
    
    Данные = Результат["data"];

	СекретныеДанные = Новый Соответствие;
	
	Для Каждого СтрокаДанных Из Данные Цикл
		
		СекретныеДанные.Вставить(СтрокаДанных["key"]
		, ?(СтрокаДанных["type"] = "Number", Число(СтрокаДанных["value"]), СтрокаДанных["value"]));
