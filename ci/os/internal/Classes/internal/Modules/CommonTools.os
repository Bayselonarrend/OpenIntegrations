...
Функция ЗапуститьВнешнееПриложение(Знач СтрокаВызова
	, Знач Каталог = Неопределено
	, Знач ВыводВКонсоль = Истина	
	, Знач Попытки = 1
	, Знач ТекущаяПопытка = 1) Экспорт
	...
	Если Процесс.КодВозврата <> 0 Тогда
		...
		Если ТекущаяПопытка >= Попытки Тогда
			ВызватьИсключение "Process failed";
		Иначе
			СообщитьПроцесс(СтрШаблон("Retrying %1/%2", ТекущаяПопытка, Попытки));
			Приостановить(2000 * ТекущаяПопытка);
			ЗапуститьВнешнееПриложение(СтрокаВызова, Каталог, ВыводВКонсоль, Попытки, ТекущаяПопытка + 1);
		КонецЕсли;
...
Функция ЗапуститьПриложениеВКонсоли(Знач СтрокаВызова
	, Знач Каталог = Неопределено
	, Знач ТолькоОшибки = Ложь
	, Знач Попытки = 1
	, Знач ТекущаяПопытка = 1) Экспорт
	...
	Если ФайлРезультата.Существует() Тогда
		...
		Если Не ТолькоОшибки Или КодВозврата <> 0 Тогда
	...
	Если Не КодВозврата = 0 Тогда
		...
		Иначе
			СообщитьПроцесс(СтрШаблон("Retrying %1/%2", ТекущаяПопытка, Попытки));
			Приостановить(2000 * ТекущаяПопытка);
			ЗапуститьПриложениеВКонсоли(СтрокаВызова, Каталог, ТолькоОшибки, Попытки, ТекущаяПопытка + 1);
		КонецЕсли;
...
Функция ЗапуститьПриложениеВКонсолиСРезультатом(Знач СтрокаВызова, Знач Каталог = Неопределено) Экспорт
	...
	Если Найти(ВРег(Система.ВерсияОС), "WINDOWS") > 0 Тогда
		СтрокаЗапуска = СтрШаблон("cmd /c ""chcp 65001>nul & %1> %2 2>&1""", СтрокаВызова, ИВФ);
	Иначе
		СтрокаЗапуска = СтрокаВызова + СтрШаблон("> %1 2>&1", ИВФ);
	КонецЕсли;
	
	КодВозврата = 0;
	ЗапуститьПриложение(СтрокаЗапуска, Каталог, Истина, КодВозврата);
	
	Вывод = "";
	ФайлРезультата = Новый Файл(ИВФ);
	
	Если ФайлРезультата.Существует() Тогда
		Вывод = ПолучитьСтрокуИзДвоичныхДанных(Новый ДвоичныеДанные(ИВФ));
		УдалитьФайлы(ИВФ);
	КонецЕсли;
...
Функция ПолучитьИнформациюОМодуле(Знач Модуль) Экспорт
	...
	СоответствиеПризнаков.Вставить("// Tool: "        , "ЭтоИнструментCLI");
