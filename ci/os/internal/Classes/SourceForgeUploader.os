...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено, Знач Язык = "ru")
	...
	ПолеПароля = ДанныеПроекта.ПолучитьЗначениеНастройки("sourceforge.hostUrl");
	Пароль     = ДанныеПроекта.ПолучитьЗначениеСекретныхДанных(ПолеПароля);

	УдаленныйКаталог = СтрШаблон("/home/frs/project/%1/%2");

	НастройкиSFTP = OPI_SFTP.ПолучитьНастройкиЛогинПароль(Хост, 22, Логин, Пароль);
