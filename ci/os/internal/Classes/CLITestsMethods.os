...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	Для Каждого Язык Из Языки Цикл
		...
		МодульТестов    = Корень + Язык + "/OInt/tests/Modules/" + ИмяМодуляТестов + ".os";
...
Процедура ПреобразоватьТестыВCLI(Знач Язык)
	...
	ТекстМодуля = ТекстМодуляОригинал;
	
	Если Язык = "ru" Тогда
		
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "#Использовать oint"   ,  "// #Использовать oint");	
	...
	Иначе
		...
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "#Use oint"   ,  "// #Use oint");	
