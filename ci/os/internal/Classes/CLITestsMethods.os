...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	CommonTools.СообщитьПроцесс("Converting tests to CLI tests");
	...
	Для Каждого Язык Из Языки Цикл
		...
		МодулиТестов    = НайтиФайлы(Корень + Язык + "/OInt/tests/Modules/", "OPIt_*.os");
		...
		Для Каждого МодульТестов Из МодулиТестов Цикл
			...
			Если МодульТестов.ИмяБезРасширения = "OPIt_Core" Тогда
				Продолжить;
			КонецЕсли;

			ДокументТестов = Новый ТекстовыйДокумент();
			ДокументТестов.Прочитать(МодульТестов.ПолноеИмя);
			
			ТекстТестов    = ДокументТестов.ПолучитьТекст();

			ИмяМодуляТестовCLI = СтрЗаменить(МодульТестов.ИмяБезРасширения, "OPIt_", "OPItc_");
...
Процедура ПреобразоватьТестыВCLI(Знач Язык)
	...
	ТекстМодуляОригинал = ДокументТестов.ПолучитьТекст();
	ТекстМодуля = СтрЗаменить(ТекстМодуляОригинал, "CLITestsMark = 0;", "CLITestsMark = 1;");
	...
	Если Язык = "ru" Тогда
		...
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "OPI_ПолучениеДанныхТестов.Обработать", "OPI_ПолучениеДанныхТестов.ОбработатьCLI");
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "УстановитьПризнакТестаCLI(Ложь)", "УстановитьПризнакТестаCLI(Истина)");
	...
	Иначе
		...
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "OPI_TestDataRetrieval.Process", "OPI_TestDataRetrieval.ProcessCLI");
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "SetCLITestFlag(False)", "SetCLITestFlag(True)");
	...
	Если Язык = "ru" Тогда
		...
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "#Использовать"   ,  "//#Использовать");	
	...
	Иначе
		...
		ТекстМодуля = СтрЗаменить(ТекстМодуля, "#Use"   ,  "//#Use");	
	...
	CommonTools.ЗаписатьТекст(ТекстМодуля, ПутиСохраненияМодулейCLI[1]);
...
Процедура ОбработатьОператор(Оператор, Язык, МассивЗамен, Отступ = "    ")
	...
	ИначеЕсли Оператор.Тип = "ОператорПрисваивания" Тогда
		...
		ОбъектИндекса    = ПолныйСостав[Команда];
		СписокПараметров = Новый Массив;
		
		Для Каждого МетодИндекса Из ОбъектИндекса["methods"] Цикл
			Если МетодИндекса["id"] = вРег(Метод) Тогда
				СписокПараметров = МетодИндекса["params"];
				Прервать;
			КонецЕсли;
		КонецЦикла;
