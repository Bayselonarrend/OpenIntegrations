...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено, Знач Язык = "ru")
	...
	Для Каждого Сборка Из СоответствиеСборки Цикл
		...
		CommonTools.ЗапуститьПриложениеВКонсоли(СтрШаблон("%1 build", OPM), ПутьПакета);
		CommonTools.ЗапуститьПриложениеВКонсоли(СтрШаблон("%1 install %2", OPM, ИмяПакета), ПутьПакета);
