...
Процедура СоздатьAppImage(Данные)
	...
	СоздатьКаталог(СтрШаблон("%1/usr", AppDir));
	СоздатьКаталог(СтрШаблон("%1/usr/share/metainfo", AppDir));
	СоздатьКаталог(СтрШаблон("%1/usr/lib/oint/bin", AppDir));

	ФС.КопироватьСодержимоеКаталога("./ci/installer_set", СтрШаблон("%1/usr", AppDir));
	
	// Копируем движок oscript
