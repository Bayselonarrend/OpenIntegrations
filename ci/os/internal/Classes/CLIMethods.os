...
Функция РазобратьМодуль(Язык, Модуль)
    ...
    ЭтоИнструментCLI = ЗначениеЗаполнено(ИнформацияМодуля["ЭтоИнструментCLI"]);
    ...
    ТекущийИндекс.Вставить("cli_tool"    , ЭтоИнструментCLI);

    Если ЭтоИнструментCLI Тогда
        ПутьВПакете = СтрШаблон("tools/Modules/%1.os", ТекущийМодуль);
    Иначе
        
        ПутьВПакете  = ИнформацияМодуля["МодульOS"];
        МассивЧастей = СтрРазделить(ПутьВПакете, "/");
        
        МассивЧастей.Удалить(0);
        МассивЧастей.Удалить(0);
        
        ПутьВПакете = СтрСоединить(МассивЧастей, "/");
        
    КонецЕсли;

    ТекущийИндекс.Вставить("path", ПутьВПакете);
        
    МассивМетодов        = Новый Массив;
    СписокМетодовРегиона = Новый СписокЗначений;
    ...
    Для Каждого Метод Из СтруктураМодуля.Объявления Цикл
        ...
        Если Метод.Тип = "ИнструкцияПрепроцессораОбласть" Тогда
            ...
            Если ЗначениеЗаполнено(СписокМетодовРегиона) Тогда
                
                СписокМетодовРегиона.СортироватьПоПредставлению();
                СписокМетодовРегиона.СортироватьПоЗначению();
                
                МассивМетодовРегиона = Новый Массив;
                
                Для Каждого Часть Из СписокМетодовРегиона Цикл
                    МассивМетодовРегиона.Добавить(Новый Структура("name,nocli", Часть.Представление, Часть.Значение));
                КонецЦикла;
        ...
        Если Метод.Тип = "ОбъявлениеМетода" И Метод.Сигнатура.Экспорт = Истина Тогда
            ...
            СписокМетодовРегиона.Добавить(ТекущийМетод["nocli"], ИмяМетода);
...
Функция РазобратьКомментарийМетода(Язык, ТекстовыйДокумент, Метод, Модуль)
    ...
    Если СтрНайти(МассивКомментария[0], "!NOCLI") > 0 Тогда
        ТекущийМетод.Вставить("nocli", Истина);
    Иначе
        ТекущийМетод.Вставить("nocli", Ложь);
    КонецЕсли;
...
Функция СформироватьСтруктуруОписанияПараметра(Знач Язык, Знач Метод, Знач ПараметрМетода, СписокЗанятыхБукв)
    ...
    ТекущееОписание.Вставить("title"      , Имя1С);
