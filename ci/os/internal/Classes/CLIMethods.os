...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
    ...
    Для Каждого Язык Из Языки Цикл
        ...
        КаталогMCP             = Корень + Язык + "/cli/data/Classes/mcp";
        КаталогДополненийОпций = КаталогДополнений;
        
        ОбъектИндекса = Новый Файл(КаталогИндекса);
        ОбъектMCP     = Новый Файл(КаталогMCP);
        
        Если Не ОбъектИндекса.Существует() Тогда
            СоздатьКаталог(КаталогИндекса);
        КонецЕсли;

        Если Не ОбъектMCP.Существует() Тогда
            СоздатьКаталог(КаталогMCP);
        КонецЕсли;
        
        СоздатьФайлОсновныхДанных(Язык);
        ЗаполнитьТаблицыСостава(Язык);

    КонецЦикла;
    
КонецПроцедуры

Процедура СоздатьФайлОсновныхДанных(Язык)
...
Процедура ЗаполнитьТаблицыСостава(Язык)
    ...
    Для Каждого Модуль Из ОбщийМассивМодулей Цикл
        ...
        Если Не ИмяКоманды = Неопределено Тогда    
            ...
            ТекущийМодуль = РазобратьМодуль(Язык, Модуль);
...
Функция РазобратьМодуль(Язык, Модуль)
    ...
    ИнформацияМодуля = CommonTools.ПолучитьИнформациюОМодуле(Модуль.ПолноеИмя);
    Зависимости      = ИнформацияМодуля["Зависимости"];

    Если Зависимости <> Неопределено Тогда
        Зависимости = СтрЗаменить(Зависимости, " ", "");
        Зависимости = СтрРазделить(Зависимости, ",");
    КонецЕсли;
    ...
    ТекущийИндекс.Вставить("dependencies", Зависимости);
    ...
    Для Каждого Метод Из СтруктураМодуля.Объявления Цикл
        ...
        Если Метод.Тип = "ОбъявлениеМетода" И Метод.Сигнатура.Экспорт = Истина Тогда
            ...
            ТекущийМетод = РазобратьКомментарийМетода(Язык, ДокументМодуля, Метод, Модуль);	 
    ...
    ДополнитьМассивИнструментов(МассивМетодов, ТекущаяБиблиотека, Язык);

    Возврат ТекущийИндекс;
    
КонецФункции

Функция РазобратьКомментарийМетода(Язык, ТекстовыйДокумент, Метод, Модуль)
    ...
    Для Каждого СтрокаПараметра Из МассивПараметров Цикл
        ...
        ТекущееОписание = СформироватьСтруктуруОписанияПараметра(Язык, Метод, СтрокаПараметра, СписокЗанятыхБукв);

        Если Не ТекущееОписание = Неопределено Тогда
...
Функция СформироватьСтруктуруМетода(Знач МассивКомментария, МассивПараметров)
    ...
    Для Каждого СтрокаКомментария Из МассивКомментария Цикл
        ...
        Если ЗаписыватьОписание = Истина И Счетчик > 1 Тогда
            ОписаниеМетода = ?(ЗначениеЗаполнено(ОписаниеМетода), ОписаниеМетода + Символы.ПС, ОписаниеМетода) 
            + СтрокаКомментария;
        КонецЕсли;
...
Функция СформироватьСтруктуруОписанияПараметра(Знач Язык, Знач Метод, Знач ПараметрМетода, СписокЗанятыхБукв)
    ...
    ЗначениеУМ = ПолучитьЗначениеПараметраПоУмолчанию(Язык, Имя1С, Метод);
    ...
    Если Имя = "--out" Тогда
        ...
        ТекущееОписание.Вставить("short", Неопределено);
        
    ИначеЕсли СписокЗанятыхБукв.НайтиПоЗначению(Буква) = Неопределено Тогда
...
Функция ПолучитьЗначениеПараметраПоУмолчанию(Знач Язык, Знач Имя, Знач Метод)
    ...
    Для Каждого ПараметрМетода Из Метод.Сигнатура.Параметры Цикл
        ...
        Если ПараметрМетода.Имя = Имя Тогда
            ...
            Если ЗначениеЗаполнено(ЗначениеПараметра) Тогда
                ...
                Иначе
                    ПустоеЗначение = ?(Язык = "ru", "Пустое значение", "Empty value");
                    Значение = ?(ЗначениеЗаполнено(Значение), Значение, ПустоеЗначение);
                КонецЕсли;
...
Функция ПолучитьИмяИнструмента(Библиотека, Значение, Язык)
	
	ПереведенноеИмя = Словарь[Значение];
	
	Если Не ЗначениеЗаполнено(ПереведенноеИмя)  Тогда
		Если Язык = "ru" Тогда
			ВызватьИсключение СтрШаблон("No translation found for %1", Значение);
		Иначе 
			ПереведенноеИмя = Значение;
		КонецЕсли;
	КонецЕсли;

    ПереведенноеИмя = Синонимайзер(ПереведенноеИмя);
	
	ПереведенноеИмя = СтрЗаменить(ПереведенноеИмя, "(", "");
	ПереведенноеИмя = СтрЗаменить(ПереведенноеИмя, ")", "");
	ПереведенноеИмя = СтрЗаменить(ПереведенноеИмя, "-", " ");
    ПереведенноеИмя = СтрЗаменить(ПереведенноеИмя, " ", "_");

    ПереведенноеИмя = СтрШаблон("%1_%2", Библиотека, ПереведенноеИмя);
    ПереведенноеИмя = НРег(ПереведенноеИмя);
	
	Возврат ПереведенноеИмя;
	
КонецФункции

Процедура ДополнитьМассивИнструментов(МассивМетодов, ТекущаяБиблиотека, Язык)

    MCPИнструменты = Новый Массив;

    Для Каждого Метод Из МассивМетодов Цикл

        ТекущийИнструмент = Новый Структура;

        ИмяИнструмента      = ПолучитьИмяИнструмента(ТекущаяБиблиотека, Метод["name"], Язык);
        ОписаниеИнструмента = Метод["description"];

        СхемаВвода            = Новый Структура;
        ...
        Для Каждого Параметр Из Метод["params"] Цикл
            ...
            СтруктураПараметра.Вставить("description", Параметр["description"]);

            ИмяПараметра = СтрЗаменить(Параметр["name"], "--", "");

            ПараметрыВвода.Вставить(ИмяПараметра, СтруктураПараметра);
        ...
        СтруктураПараметраБиблиотеки = Новый Структура("type,const", "string", ТекущаяБиблиотека);
        ПараметрыВвода.Вставить("oint_lib", СтруктураПараметраБиблиотеки);

        СтруктураПараметраМетода = Новый Структура("type,const", "string", Метод["name"]);
        ПараметрыВвода.Вставить("oint_method", СтруктураПараметраМетода);

        СхемаВвода.Вставить("type"      , "object");
        СхемаВвода.Вставить("properties", ПараметрыВвода);
        СхемаВвода.Вставить("required"  , ОбязательныеПараметры);

        ТекущийИнструмент.Вставить("name"       , ИмяИнструмента);
        ТекущийИнструмент.Вставить("description", ОписаниеИнструмента);
        ТекущийИнструмент.Вставить("inputSchema", СхемаВвода);

        MCPИнструменты.Добавить(ТекущийИнструмент);

    КонецЦикла;

    ЗаписатьИнструментыMCP(ТекущаяБиблиотека, MCPИнструменты);

КонецПроцедуры

Процедура ЗаписатьИнструментыMCP(ТекущаяБиблиотека, MCPИнструменты)

    Путь = ОбъединитьПути(КаталогMCP, СтрШаблон("%1.json", ТекущаяБиблиотека));

    CommonTools.ЗаписатьФайлJSON(Путь, Новый Структура("tools", MCPИнструменты));
