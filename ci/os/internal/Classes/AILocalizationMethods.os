...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	Для Каждого Язык Из Языки Цикл
		...
		Для Каждого ФайлИнструкции Из ФайлыИнструкций Цикл
			...
			Если ФайлИнструкцииОригинал.ПолучитьВремяИзменения() > ФайлИнструкции.ПолучитьВремяИзменения() Тогда
...
Процедура ДоработатьПереводФайлов(Язык, ФайлыКПереводу, ФайлыПримеров)
	...
	Для Каждого ФайлКПереводу Из ФайлыКПереводу Цикл
		...
		ТекстЛокализация = ПолучитьСтрокуИзДвоичныхДанных(Новый ДвоичныеДанные(ФайлКПереводу.Значение));

		ОсновнойПромпт = СтрШаблон(ШаблонПромпта, ПредставлениеЯзыка, ТекстОригинала, ТекстЛокализация);
