...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	Попытка
		Если Результат = Неопределено Или Результат.КодСостояния > 399 Тогда
			CommonTools.СообщитьПроцесс("SonarQube server unavailable. Skip...");
			Возврат;
		КонецЕсли;
	Исключение
		CommonTools.СообщитьПроцесс(Результат.ПолучитьЛог(Истина));
		Возврат;
	КонецПопытки;
	...
	CommonTools.ЗапуститьПриложениеВКонсоли(СтрокаЗапуска, КаталогИсходников);
