...
Процедура ОтформатироватьМодуль(Знач ПутьКМодулю) Экспорт
	...
	УдалитьПустыеСтроки(ТекстовыйДокумент);
...
Функция УдалитьПустыеСтроки(ТекстовыйДокумент)
	
	ПредыдущаяПустая = Ложь;
	
	Н = 1;
	
	Пока Н <= ТекстовыйДокумент.КоличествоСтрок() Цикл
		
		ТекущаяСтрока = ТекстовыйДокумент.ПолучитьСтроку(Н);
		
		Если Не ЗначениеЗаполнено(СокрЛП(ТекущаяСтрока)) Тогда
			
			Если ПредыдущаяПустая Тогда

				ПредыдущаяПустая = Ложь;
				ТекстовыйДокумент.УдалитьСтроку(Н);
				Н = Н - 2;
...
Процедура ОбработатьКомментарий(ТекстовыйДокумент, Знач Метод)
	...
	Для Н = 0 По МассивКомментария.ВГраница() Цикл
		...
		ТекущаяСтрока = МассивКомментария[Н];
		...
		Если СтрокиПараметров Тогда
			...
			МассивЧастей = СтрРазделить(ТекущаяСтрока, "-");

			Для Г = 0 По МассивЧастей.ВГраница() Цикл
				МассивЧастей[Г] = СокрЛП(МассивЧастей[Г]);
			КонецЦикла;
		...
		Иначе
			...
			Если СтрНайти(ТекущаяСтрока, "Параметры:") > 0 
				Или СтрНайти(ТекущаяСтрока, "Parameters:") > 0 Тогда
	...
	Для Каждого Замена Из СоответствиеЗамен Цикл
		...
		СтрокаПараметра = СокрЛП(СтрСоединить(ЧастиПараметраОбработанные, " - "));
	...
	Для Ц = НомерСтрокиКомментария + 1 По НомерСтроки - 1 Цикл
		...
		ТекстовыйДокумент.ЗаменитьСтроку(Ц, МассивКомментария[Счетчик]);
		Счетчик = Счетчик + 1;
