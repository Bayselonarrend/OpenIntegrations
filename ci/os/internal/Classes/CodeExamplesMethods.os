...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	CommonTools.СообщитьПроцесс("Creating code example for documentation");
	...
	Для Каждого Язык Из Языки Цикл
		...
		Для Каждого МодульТестов Из МодулиТестов Цикл
			...
			ПолучитьПримерыКодаДокументации(МодульТестов.ПолноеИмя, КаталогПримеров, КаталогЗначенийТестов, ИмяОбласти);
...
Процедура РазобратьМетод(НомерСтроки, Имя)
	...
	CommonTools.ЗаписатьТекст(ТекстПримера, ПутьПримера);
