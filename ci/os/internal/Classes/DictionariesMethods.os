...
Функция ОпределитьНаборФайловДляСловаря(Знач Корень)
	...
	МассивИсключаемыхРасширений.Добавить(".png");
	МассивИсключаемыхРасширений.Добавить(".form");
	МассивИсключаемыхРасширений.Добавить(".sbsl");
	
	Для Каждого ФайлПроекта Из ВсеФайлы Цикл
		
		Признак = 
		СтрНайти(ФайлПроекта.ПолноеИмя, "index") = 0
		И СтрНайти(ФайлПроекта.ПолноеИмя, "index") = 0
		И СтрНайти(ФайлПроекта.ПолноеИмя, ".metadata") = 0
		И СтрНайти(ФайлПроекта.ПолноеИмя, ".sonarlint") = 0
		И СтрНайти(ФайлПроекта.ПолноеИмя, "TestIB") = 0
		И СтрНайти(ФайлПроекта.ПолноеИмя, "YAXUNIT") = 0
		И СтрНайти(ФайлПроекта.ПолноеИмя, "OPI_Text_LicenseRU") = 0
