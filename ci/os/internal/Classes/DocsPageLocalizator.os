...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено, Знач Язык_ = Неопределено)
	...
	Если Язык_ = Неопределено Тогда
		Попытка
			Язык = АргументыКоманднойСтроки[0];
		Исключение
			CommonTools.СообщитьПроцесс("Language not specified for documentation page localization");
		КонецПопытки;
	Иначе
		Язык = Язык_;
	КонецЕсли;
	...
	ПутьКСловарю  = СтрШаблон("%1%2.json", КаталогСловарей, Язык);
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		CommonTools.ЗаписатьТекст(ТекущийТекст, Файл.ПолноеИмя);
