...
Процедура СоздатьЭлементJenkins(Знач Путь, Знач Имя, Знач Данные)
	...
	Если Результат >= 400 Тогда
		CommonTools.СообщитьПроцесс(СтрШаблон("Jenkins: Ошибка создания %1: %2", Путь, Результат));
	КонецЕсли;
...
Процедура УдалитьЭлементJenkins(Знач Путь)
	...
	Если Результат >= 400 Тогда
		CommonTools.СообщитьПроцесс(СтрШаблон("Jenkins: Ошибка удаления %1: %2", Путь, Результат));
	КонецЕсли;
