...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	ДанныеПроекта.ЗаписатьЗначениеНастройки("tests.availability", ДанныеКонфигурацииТестов, "./ci/config.json");
...
Процедура СформироватьWorkflow(Знач КаталогWorkflow
	...
	ТекстJFCLI   = СтрШаблон(ШаблонCliWindows, Язык, ТекстВыполненияCLI, КаталогOscript, Версия);
	ТекстJFOS    = СтрШаблон(ШаблонOsWindows, Язык, ТекстВыполненияOS, КаталогOscript, Версия);

	ТекстJFOSRPM = СтрШаблон(ШаблонOsLinux, Язык, ТекстВыполненияOS, "Rpm-Agent", Версия);
	ТекстJFOSDEB = СтрШаблон(ШаблонOsLinux, Язык, ТекстВыполненияOS, "Deb-Agent", Версия);
	
	ТекстJFCLIRPM = СтрШаблон(ШаблонCliRpm, Язык, ТекстВыполненияCLI, "Rpm-Agent", Версия);
	ТекстJFCLIDEB = СтрШаблон(ШаблонCliDeb, Язык, ТекстВыполненияCLI, "Deb-Agent", Версия);

	ТекстJF1CWin  = СтрШаблон(Шаблон1cWindows, ТекстВыполнения1С, Путь1с, Сервер1с, База);
	ТекстJF1CfWin = СтрШаблон(Шаблон1cfWindows, ТекстВыполнения1С, Путь1с, Язык);
