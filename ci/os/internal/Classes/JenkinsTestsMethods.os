...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	ДанныеКонфигурацииТестов = OPI_Tools.ПрочитатьJSONФайл(ФайлКонфигурацииТестов, Истина);
	...
	Для Каждого Язык Из Языки Цикл
		...
		СформироватьWorflow(КаталогWorkflow, Язык, СписокБиблиотек, ПутьJenkins);
		...
		Для Каждого Библиотека Из СписокБиблиотек Цикл
			...
			СформироватьWorflow(СтрШаблон("%1%2/%3/", КаталогWorkflow, "split_tests", ИмяБиблиотеки)
				, Язык
				, ТекущийСписок
