...
Процедура СоздатьЗаглушкиИнструкций()
	...
	Для Каждого Библиотека Из ИменаБиблиотек Цикл
		Если СписокИнструкций.НайтиПоЗначению(нРег(Библиотека)) = Неопределено Тогда
			...
			CommonTools.ЗаписатьТекст(СтрШаблон(ШаблонИнструкции, Библиотека, ТекущиеТеги)
				, КаталогИнструкцийЛокализация + СтрШаблон("/%1.md", Библиотека));
...
Процедура СоздатьФайлSidebars()
	...
	Для Каждого Категория Из СписокКатегорий Цикл
		...
		Для Каждого Данные Из МассивДанныхМодулей Цикл
			...
			Если Данные["Категория"] = Категория Тогда
