...
#Использовать "../../../../src/ru/OInt/api/openai"
...
Перем ЗаменыРегистра;
Перем ИндексНастроекРасширенногоВызова;
...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	Для Каждого Язык Из Языки Цикл
		...
		ТекущийЯзык         = Язык;
		ИндексНастроекРасширенногоВызова = Неопределено;
...
Процедура СоздатьСтраницуДокументации(СтруктураМетода, Счетчик = 1)
	...
	СтрокаSidebarClassName = ?(СтруктураМетода["ЕстьCLI"], "", "sidebar_class_name: doc-no-cli");
	...
	Макет = СтрЗаменить(Макет, "@СтрокаSidebarClassName", СтрокаSidebarClassName);
	...
	Макет = СтрЗаменить(Макет, "@СодержимоеВкладкиРасширенногоВызова"
		, СформироватьСодержимоеВкладкиРасширенногоВызова(СтруктураМетода["ИмяМодуля"], СтруктураМетода["ИмяМетода"]));
...
Процедура СоздатьЗаглушкиИнструкций(Знач КаталогИнструкций)
	...
	КаталогМодулей       = ДанныеПроекта.ПолучитьЗначениеНастройки("paths.configurationSrc");
	ШаблонКаталогаМодуля = ОбъединитьПути(КаталогМодулей, "CommonModules", "%1", "Module.bsl");
	...
	Для Каждого Библиотека Из ИменаБиблиотек Цикл
		Если СписокИнструкций.НайтиПоЗначению(нРег(Библиотека)) = Неопределено Тогда
			...
			Если ИспользоватьAI Тогда
				...
				ПутьМодуля  = СтрШаблон(ШаблонКаталогаМодуля, СтрШаблон("OPI_%1", Библиотека));
...
Процедура СоздатьФайлыIndexИSidebars()
	...
	Для Каждого Категория Из СписокКатегорий Цикл
		...
		Для Каждого Данные Из МассивДанныхМодулей Цикл
			...
			Если Данные["Категория"] = Категория.Ключ Тогда
				...
				ТекущаяБиблиотека = Данные["Библиотека"];
				
				Попытка
					СимволЧисло  = Число(Лев(ТекущаяБиблиотека, 1));
					ТекущийКласс = СтрШаблон("a%1", ТекущаяБиблиотека);
				Исключение
					ТекущийКласс = ТекущаяБиблиотека;
				КонецПопытки;

				Представление   = Данные["Представление" + ТРег(ТекущийЯзык)];
				ТекущееОписание = СтрШаблон(ТекстШаблонаБиблиотеки, ТекущаяБиблиотека, Представление, ТекущийКласс);
				СписокБиблиотек.Добавить(ТекущееОписание, ТекущаяБиблиотека);
	...
	// ОбновитьБлокНавигации(ОбщийМассивБиблиотек);
...
Процедура ДополнитьФайлСтиля()
	...
	Для Каждого ИмяБиблиотеки Из ИменаБиблиотек Цикл
		...
		Попытка
			СимволЧисло  = Число(Лев(ИмяБиблиотеки, 1));
			ТекущийКласс = СтрШаблон("a%1", ИмяБиблиотеки);
		Исключение
			ТекущийКласс = ИмяБиблиотеки;
		КонецПопытки;
		
		МассивКлассов.Добавить(СтрШаблон(".%1 a::before", ТекущийКласс));
		
		МассивСтилей.Добавить(СтрШаблон(".%1 > div > a::before{
		|  content: url(../../static/img/APIs/small/%2.png);
		|}", ТекущийКласс, ИмяБиблиотеки));
...
Процедура ОбновитьБлокНавигации(ОбщийМассивБиблиотек)
...
Функция Синонимайзер(ИмяРеквизита)
	...
	Для Н = 1 По МассивСлов.ВГраница() Цикл
		...
		ЗаменаРегистра = ЗаменыРегистра.Получить(нРег(ТекущееСлово));
		
		Если ЗначениеЗаполнено(ЗаменаРегистра) Тогда
			МассивСлов[Н] = ЗаменаРегистра;
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьЗаменыРегистра()

	ЗаменыРегистра = Новый Соответствие();
	ЗаменыРегистра.Вставить("atom", "Atom");
	ЗаменыРегистра.Вставить("blob", "BLOB");
	ЗаменыРегистра.Вставить("grpc", "gRPC");

КонецПроцедуры

#КонецОбласти

#Область РасширенныйВызовДокументация

Функция ПолучитьИндексНастроекРасширенногоВызова()
	
	Если ИндексНастроекРасширенногоВызова = Неопределено Тогда
		
		ПутьИндекса = "./src/" + ТекущийЯзык + "/OInt/templates/OPI_Text_MethodSettings.txt";
		ФайлИндекса = Новый Файл(ПутьИндекса);
		
		Если Не ФайлИндекса.Существует() Тогда
			ИндексНастроекРасширенногоВызова = Новый Соответствие();
		Иначе
			ЧтениеJSON = Новый ЧтениеJSON();
			ЧтениеJSON.ОткрытьФайл(ПутьИндекса);
			ИндексНастроекРасширенногоВызова = ПрочитатьJSON(ЧтениеJSON, Истина);
			ЧтениеJSON.Закрыть();
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат ИндексНастроекРасширенногоВызова;
	
КонецФункции

Функция НайтиНастройкиРасширенногоВызоваМетода(Знач ИмяМодуля, Знач ИмяМетода)
	
	Индекс = ПолучитьИндексНастроекРасширенногоВызова();
	
	НастройкиМетода = Неопределено;
	
	Попытка
		НастройкиМетода = Индекс[ИмяМодуля][ИмяМетода];
	Исключение
		НастройкиМетода = Неопределено;
	КонецПопытки;
	...
	Если НастройкиМетода.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат НастройкиМетода;
	
КонецФункции

Функция ЭкранироватьТекстЯчейкиТаблицыДокументации(Знач Текст)
...
Функция СформироватьТаблицуДопПараметровРасширенногоВызова(Знач НастройкиМетода)
	
	Шапка = ?(ТекущийЯзык = "ru"
		, "  | Параметр | Описание |"
		, "  | Parameter | Description |");
	
	Разделитель = "  |---|---|";
	Результат   = Шапка + Символы.ПС + Разделитель + Символы.ПС;
	
	Для Каждого КлючИЗначение Из НастройкиМетода Цикл
		
		ИмяПараметра = ЭкранироватьТекстЯчейкиТаблицыДокументации(КлючИЗначение.Ключ);
		Описание     = ЭкранироватьТекстЯчейкиТаблицыДокументации(КлючИЗначение.Значение);
		
		Результат = Результат + "  | " + ИмяПараметра + " | " + Описание + " |" + Символы.ПС;
...
Функция СформироватьСодержимоеВкладкиРасширенногоВызова(Знач ИмяМодуля, Знач ИмяМетода)
