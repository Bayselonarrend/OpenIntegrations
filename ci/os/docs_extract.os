...
Процедура ИзвлечениеДанных()
	...
	ПутьКДокам       = "./docs/en/md";
	ПутьКПримерам    = "./docs/en/examples/";
	ПутьКРезультатам = "./docs/en/results/";
	ПутьКДанным      = "./docs/en/data/";
	...
	Для Каждого Каталог Из КаталогиБиблиотек Цикл
		...
		Для Каждого Документ Из ФайлыДокументации Цикл
			...
			Для Н = 1 По ТекущийДокумент.КоличествоСтрок() Цикл
				...
				Если СтрНачинаетсяС(ТекущаяСтрока, "*Function ") 
					Или СтрНачинаетсяС(ТекущаяСтрока, "`Function ") Тогда

					ИмяФункции = СтрЗаменить(ТекущаяСтрока, "*Function ", "");
					ИмяФункции = СтрЗаменить(ИмяФункции, "`Function ", "");
...
Процедура ОпределитьНаличиеДанных(СоответствиеДанных, Знач ТекущаяСтрока)
	...
	ЕстьДанные = СтрНайти(ТекущаяСтрока, "New") = 0 
		И СтрНайти(ТекущаяСтрока, "OPI") = 0
		И СтрНайти(ТекущаяСтрока, "Request.") = 0;
