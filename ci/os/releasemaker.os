...
Процедура НачалоРаботы()
	...
	СтруктураРус.Вставить("ПутьCLIP", ПутьКРепозиторию + "\src\en\cli");
...
Процедура СоздатьOSPX(Данные)
	...
	СборкаCLI = "cd " + ПутьCLIP + " && opm b && opm install -f " + СтандартноеИмяOSPXCLI;
...
Процедура СоздатьПакеты(Данные)
	...
	КомандаRPM     = "mono /usr/share/oint/bin/oint ""$@""";
	ПутьКомандыRPM = ПутьВыгрузки + "oint.sh";

	ПолучитьДвоичныеДанныеИзСтроки(КомандаRPM).Записать(ПутьКомандыRPM);
	...
	+ " --verbose"
	+ " oint=/usr/share/oint/bin/oint oint.sh=/usr/bin/oint";
	...
	СоответствиеДополнений.Вставить("rpm", " --rpm-os linux ");
