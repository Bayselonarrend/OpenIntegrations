...
Процедура НачалоРаботы()
	...
	ЗапуститьПриложение(СтрШаблон("rmdir /S /Q %1\Melezh", ПутьКРепозиторию));
	ЗапуститьПриложение("git clone https://github.com/bayselonarrend/Melezh & cd ./Melezh & git pull");
...
Процедура СоздатьOSPX(Данные)
	...
	СборкаCLI         = "opm b -o ""C:/"" """ + ПутьCLIP + """";

	ЗапуститьПриложение(СборкаCLI, , Истина);
	ПереместитьФайл("C:\" + СтандартноеИмяCLI, КонечныйПутьCLI);
