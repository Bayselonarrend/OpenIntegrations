...
Процедура СоздатьOSPX(Данные)
	...
	ЗапуститьПриложение("xcopy """ + ОСкрипт + "lib\oint"" """ + КонечныйПутьOint + """ /e /y /i", , Истина);
	ЗапуститьПриложение("xcopy """ + ОСкрипт + "lib\oint-cli"" """ + КонечныйПутьCLI + """ /e /y /i", , Истина);
...
Процедура СоздатьПакеты(Данные)
	...
	ХешУстановщика = ПолучитьСтрокуИзДвоичныхДанных(Новый ДвоичныеДанные(".\ci\installer_set\shrae\oint\.versionhash"));
