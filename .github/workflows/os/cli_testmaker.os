...
Процедура СообщитьСборку()
	...
	|        run: |
	|          mw oint_" + Версия + "_all.deb oint_" + Версия + "_all_ru.deb
	|
	|      - uses: actions/upload-artifact@v3
	|        with:
	|          name: oint-deb
	|          path: |
	|            oint_" + Версия + "_all_ru.deb
	|
	...
	|        run: |
	|          mw oint-" + Версия + "-1.el7.x86_64.rpm oint-" + Версия + "-1.el7.x86_64_ru.rpm
	|
	|      - uses: actions/upload-artifact@v4
	|        with:
	|          name: oint-rpm
	|          path: |
	|            ./oint-" + Версия + "-1.el7.x86_64_ru.rpm");
...
Процедура ДобавитьСоответствияОбщие()
	...
	СоответствияПараметровЗначениям.Вставить("bitrix24", СоответствиеПЗ);
