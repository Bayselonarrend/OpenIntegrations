...
description: Создать экземпляр  и другие функции для работы с NativeAPI в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
...
`Функция СоздатьЭкземпляр(Знач Хост, Знач Класс) Экспорт`
  ...
  | Класс | - | Строка | &#x2714; | Имя класса для создания экземпляра |
