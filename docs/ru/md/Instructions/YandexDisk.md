...
**access_token** используется для авторизации во всех остальных функциях библиотеки. **refresh_token** необходимо передать в функцию `ОбновитьТокен` для получения нового access_token, когда время жизни предыдущего подойдет к концу