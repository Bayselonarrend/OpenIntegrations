...
<div>
 ...
 <a href="/docs/Instructions/ZeroMQ/"><img src={require('../../static/img/APIs/ZeroMQ.png').default} width="32" class="wp_logo"/></a>
 ...
 <a href="/docs/Instructions/Lua/"><img src={require('../../static/img/APIs/Lua.png').default} width="32" class="wp_logo"/></a>
 </div>
...
<a href="/docs/Instructions/RAR/"><img src={require('../../static/img/APIs/RAR.png').default} width="32" class="wp_logo"/></a>
</div>
<a href="/docs/Instructions/Tar/"><img src={require('../../static/img/APIs/Tar.png').default} width="32" class="wp_logo"/></a>
</div>
<!--AUTOGENERATED END-->
...
Реализация ОПИ как CLI приложения, в свою очередь, повторяет функционал расширения и предоставляет доступ ко всем его методам из командной строки. Каждому параметру оригинальной функции соответствует опция, начинающаяся с символов `--`, например `--token`. Эти соответствия для каждого конкретного метода можно найти в текущей документации, а также в комментариях к функциям 1С/OneScript и встроенной справке приложения (справка по методу отображается при вызове необходимого метода без параметров, подробнее см. [Работа с CLI версией](/docs/Start/CLI-version))
...
4. **Расширенный вызов**. Для части методов доступны дополнительные настройки (прокси, таймаут, расширенный ответ HTTP и т.д.), не входящие в обычную сигнатуру. Они описаны на странице [Расширенный вызов](/docs/Start/Advanced-call) и во второй вкладке блока параметров на странице метода; рядом с названием вкладки есть ссылка «?» на эту справку
