...
- Данные типа `Binary` при чтении возвращаются как `ДвоичныеДанные`. Для записи передайте значение в структуре `{"__OPI_BINARY__": ...}`. В CLI двоичные значения в JSON-ответе кодируются в Base64
