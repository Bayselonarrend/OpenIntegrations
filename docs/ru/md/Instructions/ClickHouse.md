...
2. Подготовить данные авторизации:
   - **JWT токен** (строка) — для авторизации через JWT
...
1. Получить адрес подключения к ClickHouse gRPC интерфейсу (например `grpc://localhost:9100`)
...
Библиотека поддерживает работу с сессиями для выполнения последовательности связанных запросов:

```bsl
// Создание новой сессии
НастройкиСессии = OPI_ClickHouse.ПолучитьНастройкиСессии(, , 300);

// Выполнение запросов в рамках сессии
Результат = OPI_ClickHouse.ВыполнитьЗапрос(Соединение, Запрос, НастройкиСессии);

// Сохранение ID сессии для последующих запросов
IDСессии = Результат["headers"]["X-ClickHouse-Session-Id"];
```
