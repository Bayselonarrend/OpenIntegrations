---
sidebar_position: 4
description: Установить отметку прочтения и другие функции для работы с GreenAPI (WhatsApp) в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
keywords: [1C, 1С, 1С:Предприятие, 1С:Предприятие 8.3, API, Интеграция, Сервисы, Обмен, OneScript, CLI, GreenAPI (WhatsApp)]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Установить отметку прочтения
 Устанавливает отметку Прочитано для сообщения или всех сообщений чата



`Функция УстановитьОтметкуПрочтения(Знач ПараметрыДоступа, Знач IDЧата, Знач IDСообщения = "") Экспорт`

  | Параметр | CLI опция | Тип | Обяз. | Назначение |
  |-|-|-|-|-|
  | ПараметрыДоступа | --access | Структура Из КлючИЗначение | &#x2714; | Параметры доступа. См. [`СформироватьПараметрыДоступа`](/docs/GreenAPI/Account/Form-access-parameters) |
  | IDЧата | --chat | Строка | &#x2714; | Идентификатор чата |
  | IDСообщения | --message | Строка | &#x2716; | Идентификатор сообщения. Все сообщения, если не заполнено |

  
  Возвращаемое значение:   Соответствие Из КлючИЗначение - сериализованный JSON ответа от Green API

<br/>

:::tip
Метод в документации API: [ReadChat](https://green-api.com/docs/api/marks/ReadChat/)
:::
<br/>


```bsl title="Пример использования для 1С:Предприятие/OneScript"
    ApiUrl           = "https://7105.api.greenapi.com";
    MediaUrl         = "https://7105.media.greenapi.com";
    IdInstance       = "71051...";
    ApiTokenInstance = "425010d90e114aa6b78f0969e...";

    IDЧата      = "120363410406221140@g.us";
    IDСообщения = "BAE57938481FFDC8";

    ПараметрыДоступа = OPI_GreenAPI.СформироватьПараметрыДоступа(ApiUrl, MediaUrl, IdInstance, ApiTokenInstance);
    Результат        = OPI_GreenAPI.УстановитьОтметкуПрочтения(ПараметрыДоступа, IDЧата, IDСообщения);
```
    

 <Tabs>
  
    <TabItem value="bash" label="Bash" default>
        ```bash
            # JSON данные также могут быть переданы как путь к файлу .json
            
            oint greenapi УстановитьОтметкуПрочтения \
              --access "{'apiUrl':'***','mediaUrl':'https://7105.media.greenapi.com','idInstance':'7105187566','apiTokenInstance':'***'}" \
              --chat "120363410406221140@g.us" \
              --message "BAE591DBC7AAC69A"
        ```
    </TabItem>
  
    <TabItem value="bat" label="CMD/Bat" default>
        ```batch
            :: JSON данные также могут быть переданы как путь к файлу .json
            
            oint greenapi УстановитьОтметкуПрочтения ^
              --access "{'apiUrl':'***','mediaUrl':'https://7105.media.greenapi.com','idInstance':'7105187566','apiTokenInstance':'***'}" ^
              --chat "120363410406221140@g.us" ^
              --message "BAE591DBC7AAC69A"
        ```
    </TabItem>
</Tabs>


```json title="Результат"
{
 "setRead": true
}
```
