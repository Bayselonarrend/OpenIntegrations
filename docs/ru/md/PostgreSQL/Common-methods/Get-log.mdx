...
description: Получить лог  и другие функции для работы с PostgreSQL в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
...
```bsl title="Пример использования для 1С:Предприятие/OneScript"
    ...
    НастройкиЛогирования = OPI_PostgreSQL.ПолучитьНастройкиЛогирования(Истина, 100, ФайлЛога);
    ...
    Соединение = OPI_PostgreSQL.ОткрытьСоединение(СтрокаПодключения, НастройкиTLS, НастройкиЛогирования);

    Если Не OPI_PostgreSQL.ЭтоКоннектор(Соединение) Тогда
        ВызватьИсключение OPI_Инструменты.JSONСтрокой(Соединение);
    КонецЕсли;

    Результат = OPI_PostgreSQL.ВыполнитьЗапросSQL("SELECT 1 AS n", , , Соединение);

    Результат = OPI_PostgreSQL.ПолучитьЛог(Соединение);
```
...
{
 "logs": [
  "[2026-06-15 16:49:24.660] Connecting to PostgreSQL...",
  "[2026-06-15 16:49:24.722] Connected to PostgreSQL",
  "[2026-06-15 16:49:24.724] Execute query (params=0, force_result=false): SELECT 1 AS n",
  "[2026-06-15 16:49:24.754] Query returned 1 row(s)"
 ],
 "result": true,
 "returned": 4,
 "total": 4
