---
sidebar_position: 1
sidebar_class_name: doc-no-cli
description: Создать соединение (REQ)  и другие функции для работы с ZeroMQ в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
keywords: [1C, 1С, 1С:Предприятие, 1С:Предприятие 8.3, API, Интеграция, Сервисы, Обмен, OneScript, CLI, ZeroMQ]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import Admonition from '@theme/Admonition';

# Создать соединение (REQ) 
 Создает соединение для отправки запроса



<Tabs>
  <TabItem value="params" label="Параметры" default>

`Функция СоздатьСоединениеReq(Знач Адрес, Знач Логирование = Неопределено) Экспорт`

  | Параметр | CLI опция | Тип | Обяз. | Назначение |
  |-|-|-|-|-|
  | Адрес | - | Строка | &#x2714; | Адрес приемника |
  | Логирование | - | Структура Из КлючИЗначение | &#x2716; | Настройки логирования. См. [`ПолучитьНастройкиЛогирования`](/docs/ZeroMQ/Common/Get-logging-settings) |


<div className="return-value-note">
  <div className="return-value-note__title">Возвращаемое значение</div>
  <div className="return-value-note__value">
  Соответствие Из КлючИЗначение, Произвольный -  Объект компоненты или соответствие с информацией об ошибке
  </div>
</div>
  
  </TabItem>
  <TabItem value="extended" label={<span>Расширенный вызов{' '}<a href="/docs/Start/Advanced-call" target="_blank" rel="noreferrer" title="Справка о расширенном вызове" onClick={(e) => e.stopPropagation()}>?</a></span>}>

  | Параметр | Описание |
  |---|---|
  | addin_mode | Ручной выбор режима подключения внешней компоненты (для 1С): Изолированно, НеИзолированно |


  </TabItem>
</Tabs>


<Admonition type="caution" title="Предупреждение" className="nocli-admonition">
<div className="addin">
<strong>NOCLI:</strong> данный метод недоступен для использования в CLI версии
</div>
</Admonition>


```bsl title="Пример использования для 1С:Предприятие/OneScript"
    // Сервер
    Порт          = 5555;
    ОбъектСервера = OPI_ZeroMQ.ОткрытьПортRep(Порт);

    Если Не OPI_ZeroMQ.ЭтоОбъектКоннектора(ОбъектСервера) Тогда
        ВызватьИсключение OPI_Инструменты.JSONСтрокой(ОбъектСервера);
    КонецЕсли;

    // Клиент
    Адрес     = "tcp://127.0.0.1:5555";
    Результат = OPI_ZeroMQ.СоздатьСоединениеReq(Адрес);

    Если Не OPI_ZeroMQ.ЭтоОбъектКоннектора(Результат) Тогда
        ВызватьИсключение OPI_Инструменты.JSONСтрокой(Результат);
    КонецЕсли;

    Сообщение = СтрШаблон("ZMQ_C_REQ_%1", Формат(ТекущаяДата(), "ДФ=yyyyMMddhhmmss"));
    Данные = ПолучитьДвоичныеДанныеИзСтроки(Сообщение);

    Отправка = OPI_ZeroMQ.ОтправитьДанные(Результат, Данные, 3000);
    Проверка = OPI_ZeroMQ.ПолучитьДанные(ОбъектСервера, 3000);
```
    



```json title="Результат"
"AddIn.OPI_ZeroMQ.Main"
```
