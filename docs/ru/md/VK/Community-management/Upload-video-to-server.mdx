---
sidebar_position: 10
description: Загрузить видео на сервер и другие функции для работы с VK в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
keywords: [1C, 1С, 1С:Предприятие, 1С:Предприятие 8.3, API, Интеграция, Сервисы, Обмен, OneScript, CLI, VK]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Загрузить видео на сервер
 Загружает видео в группу с возможностью его дальнейшего использования



`Функция ЗагрузитьВидеоНаСервер(Знач Видео, Знач Наименование, Знач Описание = "", Знач Альбом = "", Знач Параметры = "") Экспорт`

  | Параметр | CLI опция | Тип | Обяз. | Назначение |
  |-|-|-|-|-|
  | Видео | --file | Строка, ДвоичныеДанные | &#x2714; | Файл видео |
  | Наименование | --title | Строка | &#x2714; | Наименование видео |
  | Описание | --description | Строка | &#x2716; | Описание видео |
  | Альбом | --album | Строка | &#x2716; | ID альбома, если необходимо |
  | Параметры | --auth | Структура из Строка | &#x2716; | JSON авторизации или путь к .json |

  
  Возвращаемое значение:   Соответствие Из КлючИЗначение - сериализованный JSON ответа от VK

<br/>

:::tip
Параметры с типом данных Двоичные данные могут принимать также пути к файлам на диске и URL
:::
<br/>



```bsl title="Пример использования для 1С:Предприятие/OneScript"
    Параметры = ПолучитьПараметрыВК();

    Видео        = "https://hut.openintegrations.dev/test_data/video.mp4"; // URL, Двоичные данные или Путь к файлу
    Наименование = "Новое видео";
    Описание     = "Описание видео";

    Результат = OPI_VK.ЗагрузитьВидеоНаСервер(Видео, Наименование, Описание, , Параметры);
```
    

 <Tabs>
  
    <TabItem value="bash" label="Bash" default>
        ```bash
            oint vk ЗагрузитьВидеоНаСервер \
              --file ""https://hut.openintegrations.dev/test_data/video.mp4"" \
              --title ""Новое видео"" \
              --description ""Описание видео"" \
              --auth ""C:\Users\bayse\AppData\Local\Temp\ldbckgfp.uzl.json""
        ```
    </TabItem>
  
    <TabItem value="bat" label="CMD/Bat" default>
        ```batch
            oint vk ЗагрузитьВидеоНаСервер ^
              --file ""https://hut.openintegrations.dev/test_data/video.mp4"" ^
              --title ""Новое видео"" ^
              --description ""Описание видео"" ^
              --auth ""C:\Users\bayse\AppData\Local\Temp\ldbckgfp.uzl.json""
        ```
    </TabItem>
</Tabs>


```json title="Результат"
{
 "video_hash": "1020acfb285067ed7d",
 "size": 1137878,
 "direct_link": "https://vkvd162.okcdn.ru/?expires=1758367449643&srcIp=93.125.42.204&pr=40&srcAg=UNKNOWN&ms=45.136.21.137&type=4&sig=l8TYDp1zPY0&ct=27&urls=45.136.20.181&clientType=14&appId=512000384397&zs=72&id=8740018326053",
 "owner_id": -218861756,
 "video_id": 456240790
}
```
