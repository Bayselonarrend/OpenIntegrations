    ...
    <TabItem value="bash" label="Bash" default>
        ```bash
            ...
            oint ollama ПолучитьОтветВКонтексте \
              --url "https://hut.openintegrations.dev/ollama" \
              --model "tinyllama" \
              --msgs "[{'role':'user','content':'What is 1C:Enterprise?'},{'role':'assistant','content':'1C:Enterprise is a powerful accounting software solution that provides easy-to-use solutions for accounting, finance, and business management. The product offers features like invoicing, receipt tracking, budgeting, cash flow management, expense reporting, payroll, sales tax management, inventory management, and more. 1C:Enterprise is an all-in-one accounting software solution that offers an intuitive user interface to make it easy for users of any level to navigate and use the software effectively. The product\u0027s comprehensive features are designed to meet the needs of small-to-medium-size businesses, helping them stay organized and streamlined, reducing costs, and improving profitability.'},{'role':'user','content':'When the first version was released?'}]" \
