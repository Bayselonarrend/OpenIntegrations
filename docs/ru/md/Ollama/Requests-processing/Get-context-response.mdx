    ...
    <TabItem value="bash" label="Bash" default>
        ```bash
            ...
            oint ollama ПолучитьОтветВКонтексте \
              --url "https://hut.openintegrations.dev/ollama" \
              --model "tinyllama" \
              --msgs "[{'role':'user','content':'What is 1C:Enterprise?'},{'role':'assistant','content':'1C:Enterprise is a comprehensive ERP software solution designed for medium and large-scale manufacturing companies. It is an enterprise resource planning (ERP) system that provides automated solutions for manufacturing, distribution, sales, purchasing, inventory management, financial accounting, human resources, quality control, and customer service. 1C:Enterprise includes a wide range of modules such as production planning, materials management, inventory management, supply chain management, accounting and finance, HR, CRM, and e-commerce solutions. It offers customization options and integrates with other applications like customer relationship management (CRM) and e-commerce platforms.'},{'role':'user','content':'When the first version was released?'}]" \
