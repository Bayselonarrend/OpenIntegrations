...
description: Получить лог  и другие функции для работы с MySQL в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
...
```bsl title="Пример использования для 1С:Предприятие/OneScript"
    ...
    НастройкиЛогирования = OPI_MySQL.ПолучитьНастройкиЛогирования(Истина, 100, ФайлЛога);
    ...
    Соединение = OPI_MySQL.ОткрытьСоединение(СтрокаПодключения, НастройкиTLS, НастройкиЛогирования);

    Если Не OPI_MySQL.ЭтоКоннектор(Соединение) Тогда
        ВызватьИсключение OPI_Инструменты.JSONСтрокой(Соединение);
    КонецЕсли;

    Результат = OPI_MySQL.ВыполнитьЗапросSQL("SELECT 1 AS n", , , Соединение);

    Результат = OPI_MySQL.ПолучитьЛог(Соединение);
```
...
{
 "logs": [
  "[2026-06-18 18:19:00.446] Connecting to MySQL...",
  "[2026-06-18 18:19:03.313] Connected to MySQL",
  "[2026-06-18 18:19:03.331] Execute query (params=0, force_result=false): SELECT 1 AS n",
  "[2026-06-18 18:19:03.423] Query returned 1 row(s)"
 ],
 "result": true,
 "returned": 4,
 "total": 4
