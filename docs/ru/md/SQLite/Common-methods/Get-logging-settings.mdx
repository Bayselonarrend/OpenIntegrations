...
description: Получить настройки логирования и другие функции для работы с SQLite в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
...
`Функция ПолучитьНастройкиЛогирования(Знач ЗаписьВПамять = Истина, Знач МаксимумСобытий = 300, Знач ПутьКФайлу = "") Экспорт`
  ...
  |-|-|-|-|-|
  | ЗаписьВПамять | --memory | Булево | &#x2716; | Запись лога в память для дальнейшего получения из объекта компоненты |
  | МаксимумСобытий | --count | Число | &#x2716; | Максимальное число событий, хранящееся в память |
  | ПутьКФайлу | --path | Строка | &#x2716; | Путь к файлу для сохранения полного лога, если необходимо |
...
```bsl title="Пример использования для 1С:Предприятие/OneScript"
    Результат = OPI_SQLite.ПолучитьНастройкиЛогирования(Истина, 100, ПолучитьИмяВременногоФайла());
```
...
```json title="Результат"
{
 "mode": "both",
 "max_entries": 100,
 "file_path": "C:\\Users\\bayselonarrend\\AppData\\Local\\Temp\\jddjghve.xmp"
}
```
