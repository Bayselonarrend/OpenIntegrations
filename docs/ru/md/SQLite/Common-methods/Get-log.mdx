...
description: Получить лог  и другие функции для работы с SQLite в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
...
```bsl title="Пример использования для 1С:Предприятие/OneScript"
    ...
    НастройкиЛогирования = OPI_SQLite.ПолучитьНастройкиЛогирования(Истина, 100, ФайлЛога);
    ИВФ                  = ПолучитьИмяВременногоФайла("sqlite");

    Соединение = OPI_SQLite.ОткрытьСоединение(ИВФ, НастройкиЛогирования);

    Если Не OPI_SQLite.ЭтоКоннектор(Соединение) Тогда
        ВызватьИсключение OPI_Инструменты.JSONСтрокой(Соединение);
    КонецЕсли;

    Результат = OPI_SQLite.ВыполнитьЗапросSQL("SELECT 1 AS n", , , Соединение);

    Результат = OPI_SQLite.ПолучитьЛог(Соединение);
```
...
{
 "logs": [
  "[2026-07-14 10:35:08.749] Connecting to SQLite (in_memory=false)",
  "[2026-07-14 10:35:08.750] Connected to SQLite",
  "[2026-07-14 10:35:08.752] Execute query (params=0, force_result=false): SELECT 1 AS n",
  "[2026-07-14 10:35:08.752] Query returned 1 row(s)"
 ],
 "result": true,
 "returned": 4,
 "total": 4
