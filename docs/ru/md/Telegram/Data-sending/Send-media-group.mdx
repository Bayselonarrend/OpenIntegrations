---
sidebar_position: 7
description: Отправить группу медиафайлов и другие функции для работы с Telegram в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
keywords: [1C, 1С, 1С:Предприятие, 1С:Предприятие 8.3, API, Интеграция, Сервисы, Обмен, OneScript, CLI, Telegram]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Отправить группу медиафайлов
 Отправляет набор файлов в чат или канал. Варианты типов медиа: audio, document, photo, video



`Функция ОтправитьМедиагруппу(Знач Токен, Знач IDЧата, Знач Текст, Знач СоответствиеФайлов, Знач Клавиатура = "", Знач Разметка = "Markdown") Экспорт`

  | Параметр | CLI опция | Тип | Обяз. | Назначение |
  |-|-|-|-|-|
  | Токен | --token | Строка | &#x2714; | Токен бота |
  | IDЧата | --chat | Строка, Число | &#x2714; | ID целевого чата или IDЧата*IDТемы |
  | Текст | --text | Строка | &#x2714; | Текст сообщения |
  | СоответствиеФайлов | --media | Соответствие из КлючИЗначение | &#x2714; | Коллекция файлов: Ключ > Файл, Значение > Тип медиа |
  | Клавиатура | --keyboard | Строка | &#x2716; | Клавиатура. См. [`СформироватьКлавиатуруПоМассивуКнопок`](/docs/Telegram/Data-sending/Form-keyboard-from-button-array) |
  | Разметка | --parsemode | Строка | &#x2716; | Вид обработки текста (HTML, Markdown, MarkdownV2) |

  
  Возвращаемое значение:   Соответствие Из КлючИЗначение - сериализованный JSON ответа от Telegram

:::tip
Метод в документации API: [sendMediaGroup](https://core.telegram.org/bots/api#sendmediagroup)
:::
<br/>


```bsl title="Пример использования для 1С:Предприятие/OneScript"
    Токен    = "6129457865:AAFyzNYOAFbu...";
    IDЧата   = "461699897";
    Текст    = "Строковое значение";
    Картинка = "https://hut.openintegrations.dev/test_data/picture.jpg";
    Видео    = "https://hut.openintegrations.dev/test_data/video.mp4";

    КартинкаПуть = ПолучитьИмяВременногоФайла("png");
    КопироватьФайл(Картинка, КартинкаПуть);

    ВидеоПуть = ПолучитьИмяВременногоФайла("mp4");
    КопироватьФайл(Видео, ВидеоПуть);

    ВидеоДД = Новый ДвоичныеДанные(ВидеоПуть);

    Медиагруппа = Новый Соответствие;
    Медиагруппа.Вставить(КартинкаПуть, "photo");
    Медиагруппа.Вставить(ВидеоДД     , "video");

    Результат = OPI_Telegram.ОтправитьМедиагруппу(Токен, IDЧата, Текст, Медиагруппа);
```
    

 <Tabs>
  
    <TabItem value="bash" label="Bash" default>
        ```bash
            # JSON данные также могут быть переданы как путь к файлу .json
            
            oint telegram ОтправитьМедиагруппу \
              --token "***" \
              --chat "***" \
              --text "Строковое значение" \
              --media "{'https://hut.openintegrations.dev/test_data/document.docx':'document','/tmp/qauhz01a.ckt.docx':'document'}"
        ```
    </TabItem>
  
    <TabItem value="bat" label="CMD/Bat" default>
        ```batch
            :: JSON данные также могут быть переданы как путь к файлу .json
            
            oint telegram ОтправитьМедиагруппу ^
              --token "***" ^
              --chat "***" ^
              --text "Строковое значение" ^
              --media "{'https://hut.openintegrations.dev/test_data/document.docx':'document','/tmp/qauhz01a.ckt.docx':'document'}"
        ```
    </TabItem>
</Tabs>


```json title="Результат"
{
 "ok": true,
 "result": [
  {
   "message_id": 19472,
   "from": {
    "id": 6129457865,
    "is_bot": true,
    "first_name": "Бот Виталий",
    "username": "sicheebot"
   },
   "chat": {
    "id": 461699897,
    "first_name": "Anton",
    "last_name": "Titovets",
    "username": "bayselonarrend",
    "type": "private"
   },
   "date": 1771018149,
   "media_group_id": "14168145194994418",
   "photo": [
    {
     "file_id": "AgACAgIAAxkDAAJLrmmG0so_zQABN2Y2qpD-IVqWPSdQXgACuRFrG6VbMUgpis3AMBD6GQEAAwIAA3MAAzoE",
     "file_unique_id": "AQADuRFrG6VbMUh4",
     "file_size": 1568,
     "width": "***",
     "height": 67
    },
    {
     "file_id": "AgACAgIAAxkDAAJLrmmG0so_zQABN2Y2qpD-IVqWPSdQXgACuRFrG6VbMUgpis3AMBD6GQEAAwIAA20AAzoE",
     "file_unique_id": "AQADuRFrG6VbMUhy",
     "file_size": 21633,
     "width": "***",
     "height": 240
    },
    {
     "file_id": "AgACAgIAAxkDAAJLrmmG0so_zQABN2Y2qpD-IVqWPSdQXgACuRFrG6VbMUgpis3AMBD6GQEAAwIAA3gAAzoE",
     "file_unique_id": "AQADuRFrG6VbMUh9",
     "file_size": 99260,
     "width": "***",
     "height": 600
    },
    {
     "file_id": "AgACAgIAAxkDAAJLrmmG0so_zQABN2Y2qpD-IVqWPSdQXgACuRFrG6VbMUgpis3AMBD6GQEAAwIAA3kAAzoE",
     "file_unique_id": "AQADuRFrG6VbMUh-",
     "file_size": 218651,
     "width": "***",
     "height": 960
    },
    {
     "file_id": "AgACAgIAAxkDAAJLrmmG0so_zQABN2Y2qpD-IVqWPSdQXgACuRFrG6VbMUgpis3AMBD6GQEAAwIAA3cAAzoE",
     "file_unique_id": "AQADuRFrG6VbMUh8",
     "file_size": 650214,
     "width": "***",
     "height": 1920
    }
   ],
   "caption": "Строковое значение"
  },
  {
   "message_id": 19473,
   "from": {
    "id": 6129457865,
    "is_bot": true,
    "first_name": "Бот Виталий",
    "username": "sicheebot"
   },
   "chat": {
    "id": 461699897,
    "first_name": "Anton",
    "last_name": "Titovets",
    "username": "bayselonarrend",
    "type": "private"
   },
   "date": 1771018149,
   "media_group_id": "14168145194994418",
   "video": {
    "duration": 5,
    "width": "***",
    "height": 480,
    "file_name": "video1",
    "mime_type": "video/mp4",
    "thumbnail": {
     "file_id": "AAMCAgADFQdpj5elbjj9BqXriRz3LBPVNOFQuQAC3JAAAtCmgEj1RrmcFf-qgwEAB20AAzoE",
     "file_unique_id": "AQAD3JAAAtCmgEhy",
     "file_size": 20106,
     "width": "***",
     "height": 240
    },
    "thumb": {
     "file_id": "AAMCAgADFQdpj5elbjj9BqXriRz3LBPVNOFQuQAC3JAAAtCmgEj1RrmcFf-qgwEAB20AAzoE",
     "file_unique_id": "AQAD3JAAAtCmgEhy",
     "file_size": 20106,
     "width": "***",
     "height": 240
    },
    "file_id": "BAACAgIAAxUHaY-XpW44_Qal64kc9ywT1TThULkAAtyQAALQpoBI9Ua5nBX_qoM6BA",
    "file_unique_id": "AgAD3JAAAtCmgEg",
    "file_size": 1137878
   }
  }
 ]
}
```
