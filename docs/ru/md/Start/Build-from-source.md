...
Собранные бинарники внешних компонент (zip и `Template.addin`) хранятся отдельно — в orphan-ветке [`addins`](https://github.com/Bayselonarrend/OpenIntegrations/tree/addins), а не в `main`/`stable`. После клонирования `main` их подтягивают `src/addins/sync-addins.bat` (Windows) или `src/addins/sync-addins.sh` (Linux/macOS). Для `stable` бинарники с `addins` могут не подойти — соберите компоненты из `src/addins` на коммите этой ветки или возьмите архивы из релиза. Подробнее — в разделе [О внешних компонентах](/docs/Start/Component-requirements#хранение-в-репозитории).
