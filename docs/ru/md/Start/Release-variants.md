...
**oint-x.x.x_lang-x86_64.AppImage** - это файл формата AppImage, позволяющий использовать OInt CLI как единый исполняемый файл без дополнительной установки. Скачанный файл необходимо сделать исполняемым через `chmod +x ./oint-x.x.x_lang-x86_64.AppImage`, после чего он может быть запущен в терминале
