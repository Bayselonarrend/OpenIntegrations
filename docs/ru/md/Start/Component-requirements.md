...
Для **Windows 7** и **Server 2008 R2** готовые компоненты выложены в отдельной [`addins-special`](https://github.com/Bayselonarrend/OpenIntegrations/tree/addins-special) ветке репозитория. Это архивы с суффиксом `_win7.zip` (например, `OPI_SSH_win7.zip`). Для их использования необходимо заменить соответствующие общие макеты (для 1С) или `zip` файлы (для OneScript) из оригинальной поставки. Кроме того, вы можете собрать эти компоненты самостоятельно (см. [README](https://github.com/Bayselonarrend/OpenIntegrations/blob/main/src/addins/special/README.md)); сборка использует nightly Rust с целевыми платформами `win7-windows-msvc` и занимает заметно больше времени, чем стандартная
...
Библиотеки, имеющие функционал связанный с TLS, для его реализации линкуются к системным библиотекам **OpenSSL** версии 3.x - `libssl.so.3` и `libcrypto.so.3`. Это относительно новая версия, которая используется по умолчанию в дистрибутивах, начиная с CENTOS 9, RHEL 9, Fedora 36, Debian 12 и Ubuntu 22.04 (около 2022-2023 г.). Для работы в более старых дистрибутивах, использующих OpenSSL 1.1 или старше, OpenSSL 3.x должен быть установлен или собран из исходников отдельно. В качестве альтернативы вы также можете заменить компоненты оригинальной поставки на их аналоги. Последние представляют собой готовые архивы `*_static-openssl.zip`  в [ветке `addins-special`](https://github.com/Bayselonarrend/OpenIntegrations/tree/addins-special): OpenSSL 3.x вшит в `.so` и не требует установки в системе. Кроме того, при необходимости, вы можете собрать такой архив из исходников самостоятельном - см. [src/addins/special](https://github.com/Bayselonarrend/OpenIntegrations/tree/main/src/addins/special)
...
:::info
Начиная с версии **2.4.0** режим подключения `Изолированно` **не используется**. Подключает всех внешних компонент осуществляется в режиме подключения `НеИзолированно`.
:::
