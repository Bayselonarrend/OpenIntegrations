...
description: Отправить сообщение  и другие функции для работы с GRPC в Открытом пакете интеграций - бесплатной open-source библиотеке интеграций для 1С:Предприятие 8, OneScript и CLI
...
`Функция ОтправитьСообщение(Знач Соединение, Знач IDПотока, Знач Запрос) Экспорт`
  ...
  | Запрос | - | Структура Из КлючИЗначение | &#x2714; | Данные запроса для отправки |
