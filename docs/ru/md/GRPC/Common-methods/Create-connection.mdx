...
`Функция ОткрытьСоединение(Знач Параметры, Знач Tls = Неопределено, Знач Логирование = Неопределено) Экспорт`
  ...
  | Логирование | - | Структура Из КлючИЗначение | &#x2716; | Настройки логирования. См. [`ПолучитьНастройкиЛогирования`](/docs/GRPC/Common-methods/Get-logging-settings) |
