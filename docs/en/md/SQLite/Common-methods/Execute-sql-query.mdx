...
:::tip
Available parameter types: String, Number, Date, Boolean, BinaryData. Binary data can also be passed as a `{'blob':File path}` structure. Binary data (BLOB) values are returned) as `BinaryData`
...
```json title="Result"
{
 "data": [
  {
   "age": 25,
   "created_at": "2026-05-26T13:55:54Z",
   "data": {
    "blob": "Base64"
   },
   "id": 1,
