...
# About release variants

The Open Integration Package is a project with three development paths: 1C, OneScript, and CLI. The main version is 1C, which is then automatically transformed into a package for OneScript, and from there into a CLI application. Each of these paths has its own distribution options - there are 10 in total: 4 for 1C, 4 for CLI, and 2 for OneScript. Descriptions of each of these are provided below

>Link to the latest release: [Click!](https://github.com/Bayselonarrend/OpenIntegrations/releases/latest)
...
**OpenIntegrations_x.x.x_lang.cfe** - the standard distribution option for the 1C configuration extension. To start working, it is sufficient to load it into the list of extensions for your specific configuration. Development is carried out in extension compatibility mode 8.3.15 and in main compatibility mode 8.3.10
...
**oint-x.x.x-1.noarch_lang.rpm** - this is an RPM package designed to install OInt in RHEL/Fedora/CentOS and other rpm-based distributions. It automatically suggests installing `libicu` - the Unicode internationalization package (ICU) required for the application engine to work

<img style={{paddingRight:'20px',marginTop:'16px'}} src={require('./img/9.png').default} width="86" align="left"/>
**oint-x.x.x_lang-x86_64.AppImage** - this is an AppImage format file that allows you to use OInt CLI as a single executable file without additional installation on Linux. The downloaded file must be made executable via `chmod +x ./oint-x.x.x_lang-x86_64.AppImage`, after which it can be launched in the terminal
...
**oint-x.x.x_lang.ospx** - this is an OSPX package intended for installation via the OPM package manager. You can use it for offline installation using the command
``` opm install -f "./OInt-x.x.x.ospx" ```.
If your machine has internet access, it is much easier to install OInt directly from the package hub. You do not need to download any files for this, just execute the command ``` opm install oint ```

<img style={{paddingRight:'20px',marginTop:'16px'}} src={require('./img/11.png').default} width="86" align="left"/>
**oint-cli-x.x.x_lang.ospx** - this is an OSPX package of the OInt CLI application intended for installation via the OPM package manager. You can use it for offline installation using the command
``` opm install -f "./oint-cli-x.x.x.ospx" ```. It can also be installed from the package hub using the command ``` opm install oint ``` (Russian version)
