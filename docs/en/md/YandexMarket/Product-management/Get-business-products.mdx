---
sidebar_position: 3
description: Get business products and other functions to work with YandexMarket in the Open Integration Package, a free open-source integration library for 1C:Enterprise 8, OneScript and CLI
keywords: [1C, 1С, 1С:Enterprise, 1С:Enterprise 8.3, API, Integration, Services, Exchange, OneScript, CLI, YandexMarket]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Get business products
 Gets the list of products of the business



`Function GetBusinessProducts(Val Token, Val AccountID, Val Filters = "", Val PageToken = "") Export`

  | Parameter | CLI option | Type | Required | Description |
  |-|-|-|-|-|
  | Token | --Key) | String | &#x2714; | Auth token (Api |
  | AccountID | --business | String, Number | &#x2714; | Business ID |
  | Filters | --filters | Structure Of KeyAndValue | &#x2716; | Product filters |
  | PageToken | --page | String | &#x2716; | Next page token in case of a large selection |

  
  Returns:  Map Of KeyAndValue - serialized JSON response from Yandex Market




 




