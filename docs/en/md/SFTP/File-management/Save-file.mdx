 ...
 <Tabs>
    ...
    <TabItem value="bash" label="Bash" default>
        ```bash
            ...
            oint sftp SaveFile \
             --conn "{'proxy':{'login':'proxyuser','password':'***','port':'8071','proxy_type':'http','server':'127.0.0.1'},'set':{'auth_type':'private_key','host':'172.33.0.13','key_path':'***','passphrase':null,'password':null,'port':'2222','pub_path':'/tmp/0fk0wd1p.e2r.tmp','username':'bayselonarrend'}}" \
             --path "pic_from_disk.png" \
             --file "/tmp/ocieo03w.lwe.bin"
        ```
    </TabItem>
