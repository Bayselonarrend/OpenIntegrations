---
sidebar_position: 3
description: Clear outgoing message queue and other functions to work with GreenAPI (Max) in the Open Integration Package, a free open-source integration library for 1C:Enterprise 8, OneScript and CLI
keywords: [1C, 1С, 1С:Enterprise, 1С:Enterprise 8.3, API, Integration, Services, Exchange, OneScript, CLI, GreenAPI (Max)]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Clear outgoing message queue
 Clears the list of messages to be sent



`Function ClearOutgoingMessageQueue(Val AccessParameters) Export`

  | Parameter | CLI option | Type | Required | Description |
  |-|-|-|-|-|
  | AccessParameters | --access | Structure Of KeyAndValue | &#x2714; | Access parameters. See FormAccessParameters |

  
  Returns:  Map Of KeyAndValue - serialized JSON response from Green API

<br/>

:::tip
Method at API documentation: [ClearMessagesQueue](https://green-api.com/v3/docs/api/queues/ClearMessagesQueue/)
:::
<br/>


```bsl title="1C:Enterprise/OneScript code example"
    ApiUrl           = "https://3100.api.green-api.com/v3";
    MediaUrl         = "https://3100.api.green-api.com/v3";
    IdInstance       = "31003...";
    ApiTokenInstance = "17915d8e0b4b4975a183478da...";

    AccessParameters = OPI_GreenMax.FormAccessParameters(ApiUrl, MediaUrl, IdInstance, ApiTokenInstance);
    Result           = OPI_GreenMax.ClearOutgoingMessageQueue(AccessParameters);
```
 




