---
sidebar_position: 1
sidebar_class_name: doc-no-cli
description: Get logging settings  and other functions to work with NativeAPI in the Open Integration Package, a free open-source integration library for 1C:Enterprise 8, OneScript and CLI
keywords: [1C, 1С, 1С:Enterprise, 1С:Enterprise 8.3, API, Integration, Services, Exchange, OneScript, CLI, NativeAPI]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import Admonition from '@theme/Admonition';

# Get logging settings 
 Returns the logging settings template of the AddIn



<Tabs>
  <TabItem value="params" label="Parameters" default>

`Function GetLoggingSettings(Val WriteToMemory = True, Val MaxEvents = 300, Val FilePath = "") Export`

  | Parameter | CLI option | Type | Required | Description |
  |-|-|-|-|-|
  | WriteToMemory | - | Boolean | &#x2716; | Write log to memory of the AddIn |
  | MaxEvents | - | Number | &#x2716; | Maximum number of events in memory |
  | FilePath | - | String | &#x2716; | Path to the log file. If filled, writes the log to the file |


<div className="return-value-note">
  <div className="return-value-note__title">Returns</div>
  <div className="return-value-note__value">
 Structure Of KeyAndValue - Logging settings
  </div>
</div>
  
  </TabItem>
  <TabItem value="extended" label={<span>Advanced call{' '}<a href="/docs/Start/Advanced-call" target="_blank" rel="noreferrer" title="About advanced call" onClick={(e) => e.stopPropagation()}>?</a></span>}>

*This method has no additional advanced call parameters.*


  </TabItem>
</Tabs>


<Admonition type="caution" title="Caution" className="nocli-admonition">
<div className="addin">
<strong>NOCLI:</strong> this method is not available in CLI version
</div>
</Admonition>



 




