    ...
    <TabItem value="bash" label="Bash" default>
        ```bash
            ...
            oint ssh ExecuteCommand \
             --conn "{'set':{'auth_type':'private_key','host':'172.33.0.13','port':'2222','username':'bayselonarrend','key_path':'***','pub_path':'/tmp/ihhy2e2h.rhq.tmp'},'proxy':{'server':'127.0.0.1','port':'8071','proxy_type':'http','login':'proxyuser','password':'***'}}" \
             --comm "whoami"
        ```
    </TabItem>
