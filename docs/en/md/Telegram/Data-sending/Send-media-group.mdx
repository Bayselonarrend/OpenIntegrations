---
sidebar_position: 7

description: Send media group and other functions to work with Telegram in the Open Integration Package, a free open-source integration library for 1C:Enterprise 8, OneScript and CLI
keywords: [1C, 1С, 1С:Enterprise, 1С:Enterprise 8.3, API, Integration, Services, Exchange, OneScript, CLI, Telegram]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import Admonition from '@theme/Admonition';

# Send media group
 Sends a set of files to a chat or channel. Media types: audio, document, photo, video



<Tabs>
  <TabItem value="params" label="Parameters" default>

`Function SendMediaGroup(Val Token, Val ChatID, Val Text, Val FileMapping, Val Keyboard = "", Val Markup = "Markdown") Export`

  | Parameter | CLI option | Type | Required | Description |
  |-|-|-|-|-|
  | Token | --token | String | &#x2714; | Bot token |
  | ChatID | --chat | String, Number | &#x2714; | Target chat ID or ChatID*TopicID |
  | Text | --text | String | &#x2714; | Message text |
  | FileMapping | --media | Map of KeyAndValue | &#x2714; | File collection: Key > File, Value > Media Type |
  | Keyboard | --keyboard | String | &#x2716; | Keyboard. See FormKeyboardFromButtonArray |
  | Markup | --parsemode | String | &#x2716; | Text processing type (HTML, Markdown, MarkdownV2) |


<div className="return-value-note">
  <div className="return-value-note__title">Returns</div>
  <div className="return-value-note__value">
 Map Of KeyAndValue - serialized JSON response from Telegram
  </div>
</div>
  
  </TabItem>
  <TabItem value="extended" label={<span>Advanced call{' '}<a href="/docs/Start/Advanced-call" target="_blank" rel="noreferrer" title="About advanced call" onClick={(e) => e.stopPropagation()}>?</a></span>}>

  | Parameter | Description |
  |---|---|
  | proxy | `InternetProxy` or a structure with fields `Protocol`, `Host`, `Port`, `User`, `Password`, `UseOSAuthentication` |
  | timeout | Request execution timeout |
  | adv_response | Formats the response as a complete HTTP structure with fields `code`, `body`, and `headers` |
  | retries | Number of HTTP request send attempts on `5**` status codes or internal client errors |
  | dontwait | Creates a background job and returns its data (for 1C and OneScript only) |


  </TabItem>
</Tabs>

:::tip
Method at API documentation: [sendMediaGroup](https://core.telegram.org/bots/api#sendmediagroup)
:::
<br/>


```bsl title="1C:Enterprise/OneScript code example"
    Token  = "6129457865:AAFyzNYOAFbu...";
    ChatID = "461699897";
    Text   = "Строковое значение";
    Image  = "https://releases.openintegrations.dev/test_data/picture.jpg";
    Video  = "https://releases.openintegrations.dev/test_data/video.mp4";

    ImagePath = GetTempFileName("png");
    CopyFile(Image, ImagePath);

    VideoPath = GetTempFileName("mp4");
    CopyFile(Video, VideoPath);

    VideoDD = New BinaryData(VideoPath);

    MediaGroup = New Map;
    MediaGroup.Insert(ImagePath, "photo");
    MediaGroup.Insert(VideoDD  , "video");

    Result = OPI_Telegram.SendMediaGroup(Token, ChatID, Text, MediaGroup);
```
 

 <Tabs>
  
    <TabItem value="bash" label="Bash" default>
        ```bash
            # JSON data can also be passed as a path to a .json file
            
            oint telegram SendMediaGroup \
              --token "***" \
              --chat "***" \
              --text "Строковое значение" \
              --media "{'https://releases.openintegrations.dev/test_data/document.docx':'document','/tmp/ovihuswb.nw0.docx':'document'}"
        ```
    </TabItem>
  
    <TabItem value="bat" label="CMD/Bat" default>
        ```batch
            :: JSON data can also be passed as a path to a .json file
            
            oint telegram SendMediaGroup ^
              --token "***" ^
              --chat "***" ^
              --text "Строковое значение" ^
              --media "{'https://releases.openintegrations.dev/test_data/document.docx':'document','/tmp/ovihuswb.nw0.docx':'document'}"
        ```
    </TabItem>
</Tabs>


```json title="Result"
{
 "ok": true,
 "result": [
  {
   "message_id": 24364,
   "from": {
    "id": 6129457865,
    "is_bot": true,
    "first_name": "Бот Виталий",
    "username": "sicheebot"
   },
   "chat": {
    "id": 461699897,
    "first_name": "Anton",
    "last_name": "Titovets",
    "username": "bayselonarrend",
    "type": "private"
   },
   "date": 1784481063,
   "media_group_id": "14275848504932690",
   "photo": [
    {
     "file_id": "AgACAgIAAxkDAAJdvGpaVPvZdG8AAbvA9S38U1j7Dy-_tgACfxprG3-z2UoHiccCrLehQAEAAwIAA3MAAz0E",
     "file_unique_id": "AQADfxprG3-z2Up4",
     "file_size": 1569,
     "width": "***",
     "height": 67
    },
    {
     "file_id": "AgACAgIAAxkDAAJdvGpaVPvZdG8AAbvA9S38U1j7Dy-_tgACfxprG3-z2UoHiccCrLehQAEAAwIAA20AAz0E",
     "file_unique_id": "AQADfxprG3-z2Upy",
     "file_size": 21511,
     "width": "***",
     "height": 240
    },
    {
     "file_id": "AgACAgIAAxkDAAJdvGpaVPvZdG8AAbvA9S38U1j7Dy-_tgACfxprG3-z2UoHiccCrLehQAEAAwIAA3gAAz0E",
     "file_unique_id": "AQADfxprG3-z2Up9",
     "file_size": 98652,
     "width": "***",
     "height": 600
    },
    {
     "file_id": "AgACAgIAAxkDAAJdvGpaVPvZdG8AAbvA9S38U1j7Dy-_tgACfxprG3-z2UoHiccCrLehQAEAAwIAA3kAAz0E",
     "file_unique_id": "AQADfxprG3-z2Up-",
     "file_size": 217083,
     "width": "***",
     "height": 960
    },
    {
     "file_id": "AgACAgIAAxkDAAJdvGpaVPvZdG8AAbvA9S38U1j7Dy-_tgACfxprG3-z2UoHiccCrLehQAEAAwIAA3cAAz0E",
     "file_unique_id": "AQADfxprG3-z2Up8",
     "file_size": 645868,
     "width": "***",
     "height": 1920
    }
   ],
   "caption": "Строковое значение"
  },
  {
   "message_id": 24365,
   "from": {
    "id": 6129457865,
    "is_bot": true,
    "first_name": "Бот Виталий",
    "username": "sicheebot"
   },
   "chat": {
    "id": 461699897,
    "first_name": "Anton",
    "last_name": "Titovets",
    "username": "bayselonarrend",
    "type": "private"
   },
   "date": 1784481063,
   "media_group_id": "14275848504932690",
   "video": {
    "duration": 5,
    "width": "***",
    "height": 480,
    "file_name": "video1",
    "mime_type": "video/mp4",
    "thumbnail": {
     "file_id": "AAMCAgADFQdqXQUmxxQVMly5ekIkx1LUzdIwFwACzq0AAmmx6UpvpGym1CAK2QEAB20AAz0E",
     "file_unique_id": "AQADzq0AAmmx6Upy",
     "file_size": 19807,
     "width": "***",
     "height": 240
    },
    "thumb": {
     "file_id": "AAMCAgADFQdqXQUmxxQVMly5ekIkx1LUzdIwFwACzq0AAmmx6UpvpGym1CAK2QEAB20AAz0E",
     "file_unique_id": "AQADzq0AAmmx6Upy",
     "file_size": 19807,
     "width": "***",
     "height": 240
    },
    "file_id": "BAACAgIAAxUHal0FJscUFTJcuXpCJMdS1M3SMBcAAs6tAAJpselKb6RsptQgCtk9BA",
    "file_unique_id": "AgADzq0AAmmx6Uo",
    "file_size": 1137878
   }
  }
 ]
}
```
