...
Этот проект предоставляет библиотеку для интеграции 1С:Предприятие, OneScript и CLI с внешними сервисами через REST API и другие протоколы. Благодаря представленным в ней методам вы можете работать в экосистеме 1С:Предприятие с такими сервисами и технологиями как: 7z, Airtable, Bitrix24, CDEK, ClickHouse, Dropbox, FTP(S), Google Calendar, Google Drive, Google Sheets, GreenAPI (WhatsApp), GreenAPI (Max), gRPC, Janx, LDAP, Lua, MessagePack, MongoDB, MS SQL, MySQL, Neocities, Notion, Ntfy, Ollama, OpenAI, PostgreSQL, RAR, RCON, ReportPortal, RSS/Atom, AWS S3, SFTP, Slack, SQLite, SSH, Tar, TCP, Telegram, Viber, VK, VK Teams, WebSocket, Yandex.Disk, ZeroMQ, HTTP
