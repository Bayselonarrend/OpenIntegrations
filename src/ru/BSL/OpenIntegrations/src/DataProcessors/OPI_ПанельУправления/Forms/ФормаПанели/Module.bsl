...
Процедура HTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Попытка
		...
		ИначеЕсли IDОбъекта = "opi-btn-sourcecraft" Тогда
			ОткрытьSourceCraft();
		ИначеЕсли IDОбъекта = "tab-label-main" Или IDОбъекта = "tab-label-addins" Тогда
			УстановитьАктивнуюВкладку(IDОбъекта);
...
&НаСервере
Процедура УстановитьАктивнуюВкладку(Знач IDВкладки)

	ФормаHTML = РеквизитФормыВЗначение("ХранилищеФормы").Восстановить(ДанныеФормы);
	
	ФормаHTML
		.УдалитьАтрибут("checked", "opi-tab-main")
		.УдалитьАтрибут("checked", "opi-tab-addins");

	ФормаHTML.УстановитьАтрибут("checked"
		, "checked"
		, СтрШаблон("opi-tab-%1", СтрЗаменить(IDВкладки, "tab-label-", "")));
...
Процедура ОбработатьОбщуюОтметку(Знач Активно)
	...
	Для Каждого Компонента Из ВерсииКомпонент Цикл
		...
		Иначе
			ФормаHTML.УдалитьАтрибут("checked", IDЭлемента);	 
		КонецЕсли;
