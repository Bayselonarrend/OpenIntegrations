...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОсновнаяФорма = Обработки.OPI_HTMLФорма.Создать().Инициализировать();
	ДанныеФормы   = ОсновнаяФорма.Сохранить();
	ЗначениеВРеквизитФормы(ОсновнаяФорма, "ХранилищеФормы");
	...
	Попытка
		...
		Если ТипСобытия = "click" Тогда
			...
			ИначеЕсли IDОбъекта = "opi-btn-sourcecraft" Тогда
				ОткрытьSourceCraft();
			Иначе
				ОбработатьСерверныеСобытияHTML(IDОбъекта, ТипСобытия, ДанныеСобытия);
			КонецЕсли;
...
&НаСервере
Процедура ОбработатьСерверныеСобытияHTML(Знач IDОбъекта, Знач ТипСобытия, Знач ДанныеСобытия)
	
	ФормаHTML = РеквизитФормыВЗначение("ХранилищеФормы").Восстановить(ДанныеФормы);
	
	Если IDОбъекта = "opi-addins-check-all" Тогда
		
		Установлен = ДанныеСобытия["Element"]["checked"];				
		ОбработатьОбщуюОтметку(ФормаHTML, Установлен);
				
	ИначеЕсли СтрНачинаетсяС(IDОбъекта, "opi-addin-OPI_") Тогда
		
		ОбработатьФлажокКомпоненты(ФормаHTML, IDОбъекта);

	КонецЕсли;
				
	ДанныеФормы = ФормаHTML.Сохранить();
				
КонецПроцедуры

&НаСервере
Процедура ОбработатьФлажокКомпоненты(ФормаHTML, Знач IDОбъекта)
...
&НаСервере
Процедура ОбработатьОбщуюОтметку(ФормаHTML, Знач Активно)
...
Процедура ПересобратьHTMLПанелиНаСервере(ПоказыватьЗагрузку = Ложь)
	...
	ФормаHTML = РеквизитФормыВЗначение("ХранилищеФормы").Инициализировать();
