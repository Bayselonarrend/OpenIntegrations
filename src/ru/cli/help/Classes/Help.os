...
Процедура ВывестиНачальнуюСтраницу(Знач СоответствиеКомандМодулей, Знач Версия) Экспорт
	...
	ОписаниеСтандартныхОпций = СтрШаблон("
		|
	...
	ЦветнойВывод.ВывестиСтроку(ОписаниеСтандартныхОпций);
...
Функция ПолучитьПолноеОписаниеПараметров(ТаблицаПараметров)
	...
	ДлинаОтступаНовойСтроки = МаксимальнаяДлина + 5;
	ОтступНовойСтроки       = "";
...
Функция ПолучитьОписаниеСПереносамиПоШирине(МассивСтрокОписания, ОтступНовойСтроки, ДлинаОтступа)
	...
	ДопустимаяДлинаСтроки = Консоль.Ширина - ДлинаОтступа - 4;
		...
		Пока Н <= МассивСлов.ВГраница() Цикл
			...
			Иначе
				...
				Если ТекущаяСтрока = "" Или СтрДлина(Слово) > ТретьДопустимой Тогда
					...
					ТекущаяСтрока = ТекущаяСтрока + ЛеваяЧасть + "-";
