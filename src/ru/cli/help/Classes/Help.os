...
Процедура ВывестиСправкуПоМетодам(Знач Команда) Экспорт
	...
	Для каждого СтрокаОбласти Из ДанныеОбластей Цикл
		...
		Для Каждого МетодОбласти Из МетодыОбласти Цикл
			...
			ЦветнойВывод.ВывестиСтроку("    (" + Метка + "|#color=Желтый) " + МетодОбласти);
...
Функция ПолучитьПолноеОписаниеПараметров(ДанныеМетода)
	...
	ШаблонОписанияПараметра   = "    (%1|#color=Желтый) -%2%3";
	...
	Для Каждого ПараметрМетода Из ПараметрыМетода Цикл
		...
		ТекущееОписание             = ПараметрыМетода["description"];
		ТекущееЗначениеПоУмолчанию  = ПараметрыМетода["default"];
		...
		МассивОписания  = СтрРазделить(ПараметрМетода["description"], Символы.ПС);
		...
		ТекущееПолноеОписание = СтрШаблон(ШаблонОписанияПараметра, ТекущийСписокОпций, ТекущееОписание, ОписаниеЗначенияПоУмолчанию);
