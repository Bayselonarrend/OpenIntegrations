...
Перем ИндексРасширенногоВызова;
...
Процедура ВывестиНачальнуюСтраницу() Экспорт
	...
	СписокИменКоманд          = Новый СписокЗначений;
	СписокИменОсобыхКоманд    = Новый СписокЗначений;
	
	Для Каждого Команда Из СоответствиеКомандМодулей Цикл

		Если СтрНачинаетсяС(Команда.Значение, "OPI_") Тогда
			СписокИменКоманд.Добавить(Команда.Ключ)
		Иначе
			СписокИменОсобыхКоманд.Добавить(Команда.Ключ)
		КонецЕсли;
		
	КонецЦикла;
	
	СписокИменКоманд.СортироватьПоЗначению();
	СписокИменОсобыхКоманд.СортироватьПоЗначению();
	
	СписокКоманд = СтрСоединить(СписокИменКоманд.ВыгрузитьЗначения(), ", ");
	СписокОсобыхКоманд = СтрСоединить(СписокИменОсобыхКоманд.ВыгрузитьЗначения(), ", ");
	...
	ЦветнойВывод.Вывести(СтрШаблон("
		...
		|                          
		|
		| Добро пожаловать в (OInt|#color=Белый) v (%1|#color=Зеленый)!
		|
		| Структура вызова:
	    | 
		...
		| Список доступных библиотек: (%2|#color=Малиновый)(, %3|#color=Белый) 
		|"
		, Версия
		, СписокОсобыхКоманд
		, СписокКоманд));
	...
	ЦветнойВывод.Вывести(СтрШаблон("
		...
		|
		|  (--help|#color=Зеленый)   -%1
		|  (--debug|#color=Зеленый)  -%2
		|  (--config|#color=Зеленый) -%3
		|  (--out|#color=Зеленый)    -%4
		|
		...
		, ПолучитьОписаниеСПереносамиПоШирине("доп. настройки вызова метода. При использовании без значения возвращает доступные", ОтступНовойСтроки, ДлинаОтступа)
...
Процедура ВывестиСправкуПоМетодам(Знач Команда) Экспорт
	...
	ШаблонМетода  = "   (%1|#color=Желтый) (%2|#color=%3)";
	...
	Для каждого СтрокаОбласти Из ДанныеОбластей Цикл
		...
		Для Каждого МетодОбласти Из МетодыОбласти Цикл
			...
			ТекущийЦвет = ?(МетодОбласти["nocli"], "ТемноСерый", "Белый");
			ЦветнойВывод.ВывестиСтроку(СтрШаблон(ШаблонМетода, Метка, МетодОбласти["name"], ТекущийЦвет));
...
Процедура ВывестиСправкуПоПараметрам(Знач Команда, Знач Метод) Экспорт 
	...
	Если ДанныеМетода["nocli"] = Истина Тогда
		Дополнение = " ((только server)|#color=ТемноСерый)";
	Иначе
		Дополнение = "";
	КонецЕсли;
	...
	| (■|#color=Зеленый) Метод (%1|#color=Бирюза)%4
	| (■|#color=Зеленый) %2
	|
	|%3
	|", ИмяМетода, ОписаниеМетода, ПолноеОписаниеПараметров, Дополнение); 
...
Процедура ВывестиСправкуПоНастройкам(Знач Команда, Знач Метод) Экспорт

	ПодключитьИндексРасширенныйВызов();

	Команды   = ОбъектОПИ.ПолучитьСоответствиеКомандМодулей();
	ИмяМодуля = Команды.Получить(Команда);

	ТекстСправки = ИндексРасширенногоВызова.ВернутьСписокНастроек(ИмяМодуля, Метод);

	Консоль.ЦветТекста = ЦветКонсоли.Желтый;
	
	Консоль.ВывестиСтроку(ТекстСправки);
...
Процедура ВывестиСообщениеИсключения(Знач Причина, Знач ФайлВывода = "") Экспорт
	...
	ИначеЕсли Причина = "NOCLI" Тогда
		Текст = "Данный метод имеет смысл только в режиме сервера (команда server)";
		Код   = 3;
...
Функция ПолучитьОписаниеСПереносамиПоШирине(МассивСтрокОписания, ОтступНовойСтроки, ДлинаОтступа)
	...
	Если Не ИспользоватьАдаптивныйВывод Тогда

		Если МассивСтрокОписания.Количество() = 1 Тогда
			ТекущееОписание = МассивСтрокОписания[0];
		Иначе
			ТекущееОписание = СтрСоединить(МассивСтрокОписания, Символы.ПС + ОтступНовойСтроки);
		КонецЕсли;
...
Процедура ПодключитьИндексРасширенныйВызов()

	Если ИндексРасширенногоВызова = Неопределено Тогда

		ТекущийКаталогСкрипта = СтрЗаменить(ТекущийСценарий().Каталог, "\", "/");

		ПутьИндекса  = СтрШаблон("%1/%2"
			, ТекущийКаталогСкрипта
			, "internal/Classes/AdvancedCallIndex.os");

		ИндексРасширенногоВызова = ЗагрузитьСценарий(ПутьИндекса);
