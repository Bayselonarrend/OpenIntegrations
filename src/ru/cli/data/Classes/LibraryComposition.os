...
Функция СформироватьСтрокуИнициализацииМодуля(Знач ДанныеКоманды, Знач Модуль)
    ...
    Если ДанныеКоманды["cli_tool"] Тогда

        ШаблонЗагрузки = СтрШаблон("%%1 = ЗагрузитьСценарий(""%1/tools/Modules/%%1.os"", Контекст);"
            , КаталогПриложения);

    Иначе
        ШаблонЗагрузки = СтрШаблон("%%1 = ЗагрузитьСценарий(""%1/oint/api/%2/Modules/%%1.os"", Контекст);"
            , КаталогПакетов
            , ДанныеКоманды["library"])
