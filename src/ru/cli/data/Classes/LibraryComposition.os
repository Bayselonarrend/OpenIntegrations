...
Функция СформироватьСтрокуВызоваМетода(Знач ПереданныеПараметры, Знач Команда, Знач Метод, Знач ИмяТекущегоОбъекта = "") Экспорт
    ...
    ЗаменитьУправляющиеПоследовательности(ТекстВыполнения, Истина);
...
Процедура ЗаменитьУправляющиеПоследовательности(Текст, ТолькоОбязательные = Ложь) Экспорт

    Текст = Строка(Текст);

    СоответствиеСимволов = ПолучитьСоответствиеУправляющихПоследовательностей(ТолькоОбязательные);
...
Функция ПолучитьСоответствиеУправляющихПоследовательностей(ТолькоОбязательные)
...
Procedure ReplaceEscapeSequences(Text, RequiredOnly = False) Export
	ЗаменитьУправляющиеПоследовательности(Text, RequiredOnly);
EndProcedure
