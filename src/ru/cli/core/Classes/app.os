...
Перем ШаблонOint;        // Шаблон пути к oint
Перем OPI_Инструменты;   // Модуль инструментов
...
Процедура ОсновнойОбработчик()
	...
	ЧастиПути.Удалить(ЧастиПути.ВГраница());
	ЧастиПути.Добавить("oint");

	ШаблонOint = СтрСоединить(ЧастиПути, "/") + "/%1";
...
Процедура ПодключитьМодульИнструментов()

	Если OPI_Инструменты = Неопределено Тогда
		ПутьИнструментов = СтрШаблон(ШаблонOint, "tools/Modules/internal/Modules/OPI_Инструменты.os");
		OPI_Инструменты  = ЗагрузитьСценарий(ПутьИнструментов);
	КонецЕсли;
