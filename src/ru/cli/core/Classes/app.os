...
Перем ЗаписьИзФункции;   // Запись в файл не нужна, так как осуществляется внутри основной функции
...
Процедура ОсновнойОбработчик()
	...
	ОбъектОПИ.УстановитьКаталоги(КаталогПакетов, КаталогПриложения);
...
Процедура ОпределитьТекущуюКомандуМетод()
	...
	Если ЧислоАргументов > 1 Тогда
		...
		Если СтрНачинаетсяС(ТекущийМетод, "-") Тогда
...
Процедура СформироватьНаборПараметров()
	...
	Иначе
		ЗаписьИзФункции = Истина;
	КонецЕсли;

	Парсер.ДобавитьИменованныйПараметрКоманды(Команда, "--config");
...
Функция ПолучитьРезультатОбработки(Знач Параметры)
	...
	Если Не ЗначениеЗаполнено(Метод) Или Метод = "--help" Тогда
		Справка.ВывестиСправкуПоМетодам(ТекущаяКоманда);
	КонецЕсли;

	Конфигурация = Параметры["--config"];

	Если Конфигурация <> Неопределено Тогда

		Если Конфигурация = "" Тогда
			Справка.ВывестиСправкуПоНастройкам(ТекущаяКоманда, Метод);
		КонецЕсли;

	КонецЕсли;

	Если Параметры.Количество() = ЧислоСтандартныхПараметров Или Параметры["--help"] Тогда
		Справка.ВывестиСправкуПоПараметрам(ТекущаяКоманда, Метод);
	КонецЕсли;

	СтруктураВыполнения = ОбъектОПИ.СформироватьСтрокуВызоваМетода(Параметры, ТекущаяКоманда, Метод);
	...
	Если Не Тестирование Тогда
		ПодключитьИсполнитель();
		Исполнитель.ВыполнитьСкрипт(ТекстВыполнения, Ответ, Конфигурация);
	КонецЕсли;
...
Функция ЗаписатьЗначениеВФайл(Знач Значение, Знач Путь)
	...
	Если Не ЗаписьИзФункции Тогда
		ПодготовитьЗапись(Значение, Путь);
	КонецЕсли;
	...
	Если ЗаписанныйФайл.Существует() Тогда
		...
		Возврат СтрШаблон("Файл размером %1 МБ был записан в %2"
...
Процедура ОпределитьШаблоныПутей()
	...
	ШаблонДоступа     = КаталогПриложения + "/%1";
...
Функция ПодготовитьВыводCLI(Знач Данные)
	...
	Если ТипДанных = Тип("ДвоичныеДанные") Тогда
		Возврат ПолучитьBase64СтрокуИзДвоичныхДанных(Данные);
	КонецЕсли;
	...
	Если ТипДанных = Тип("Массив") Тогда
		...
		Для Каждого Элемент Из Данные Цикл
			Результат.Добавить(ПодготовитьВыводCLI(Элемент));
		КонецЦикла;
	...
	Если ТипДанных = Тип("Структура") Или ТипДанных = Тип("Соответствие") Тогда
		...
		Для Каждого Пара Из Данные Цикл
			Результат.Вставить(Пара.Ключ, ПодготовитьВыводCLI(Пара.Значение));
		КонецЦикла;
...
Процедура ОбработатьВыводJSON(Вывод)
	...
	Если ТипЗнч(Вывод) = Тип("Структура")
		...
		Вывод = JSONСтрокой(ПодготовитьВыводCLI(Вывод));
...
Процедура СообщитьРезультат(Знач Текст, Знач Статус = "")
	...
	Если ЗначениеЗаполнено(ФайлВывода) Тогда
		...
		ФайлВыводаНормализованный = СокрЛП(Строка(ФайлВывода));

		КакПредставление = СтрНачинаетсяС(ФайлВыводаНормализованный, "_") 
			И СтрЗаканчиваетсяНа(ФайлВыводаНормализованный, "_");
		
		Если КакПредставление Тогда
			
			Текст = ПолучитьПредставлениеДвоичныхДанных(Текст, ФайлВыводаНормализованный);
			
		Иначе
			
			Текст = ЗаписатьЗначениеВФайл(Текст, ФайлВыводаНормализованный);

		КонецЕсли;

	ИначеЕсли ТипЗнч(Текст) = Тип("ДвоичныеДанные") Тогда

		Текст = "Похоже, что в ответе пришли двоичные данные! В следующий раз используйте опцию --out для указания пути их сохранения";
...
Процедура ПодготовитьЗапись(Значение, Путь)
...
Функция ПолучитьПредставлениеДвоичныхДанных(Знач Текст, Знач ФайлВывода)

	Значение = ?(ТипЗнч(Текст) = Тип("ДвоичныеДанные"), Текст, ПолучитьДвоичныеДанныеИзСтроки(Строка(Текст)));
	
	Если ФайлВывода = "_base64_" Тогда
		Представление = ПолучитьBase64СтрокуИзДвоичныхДанных(Значение);
	ИначеЕсли ФайлВывода = "_utf8_" Тогда
		Представление = ПолучитьСтрокуИзДвоичныхДанных(Значение);
	ИначеЕсли ФайлВывода = "_hex_" Тогда
		Представление = ПолучитьHexСтрокуИзДвоичныхДанных(Значение);
	Иначе
		Представление = ЗаписатьЗначениеВФайл(Значение, ФайлВывода);
	КонецЕсли;
