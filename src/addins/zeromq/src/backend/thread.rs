...
use zeromq::{PubSocket, PullSocket, PushSocket, RepSocket, ReqSocket, Socket, SocketRecv, SocketSend, SubSocket, ZmqMessage};
...
pub(crate) fn spawn_thread(logger: Option<Arc<Logger>>) -> Result<BackendThread<BackendCommand>, String> {
    BackendThread::spawn("opi_zeromq_backend", move |rt, rx| {
        let mut state = BackendState::new(logger);
        ...
        while let Ok(cmd) = rx.recv() {
            match cmd {
                ...
                BackendCommand::IsBusy { response } => {
                    let _ = response.send(state.is_busy());
                }
