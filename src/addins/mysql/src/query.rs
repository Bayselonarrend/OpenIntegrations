...
fn from_sql_to_json(value: mysql::Value, column: &mysql_common::packets::Column) -> Value {
    match value {
...
fn process_mysql_params(
    binary_vault: &BinaryVault,
    json_array: &mut Vec<Value>,
) -> Result<Vec<mysql::Value>, String> {
                ...
                if let Some((key, value)) = obj.iter().next() {
                    match key.to_uppercase().as_str() {
                        "BYTES" => match value.as_str() {
                            Some(key) => {
                                let key_obj =
                                    &VaultKey::from_str(key).map_err(|e| e.to_string())?;

                                let binary =
                                    binary_vault.retrieve(key_obj).map_err(|e| e.to_string())?;
                        ...
                        "TEXT" => {
                            ...
                            match value_str {
                                Some(value_str) => {
                                    mysql::Value::Bytes(value_str.as_bytes().to_vec())
                                }
